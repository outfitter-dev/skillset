...
If the optional project SessionStart hook reports stale generated output, use `skillset-help` for the build and verification walkthrough; the hook only advises and never rebuilds automatically. `compile.session_start_hook` selects `auto`, `on`, or `off`, and its project-local settings entry is separate from plugin hooks.
...
skillset new rule "Review Guidance" --yes
...
Use `.skillset/rules/**/*.md` for durable repo rules:
