...
Use `.skillset/skills/`, `.skillset/rules/`, and `.skillset/agents/` for working inside a project; no plugin or marketplace is required. Applicable rules generate root or scoped `AGENTS.md`, and portable skills generate `.agents/skills/`, independently of provider selection. Claude, Codex, and Cursor are enabled by default. Root `claude: false` controls Claude output only. Codex-specific output includes project-agent TOML under `.codex/agents/`, skill sidecars such as `agents/openai.yaml`, native command-policy files, and ChatGPT product bundles/catalogs when plugin source exists.

In a mixed workspace, `skillset build --scope repo,project` selects local skill roots, instructions, project agents, and provider source while excluding plugin bundle destinations. This is not a private/public content boundary: eligible plugin-owned skills still participate in `.agents/skills/`. Root-level `skills/`, `hooks/`, and `rules/` are not additional automatic source roots. Import copies recognized content into `.skillset/`; it does not link the original directory. Only explicitly supported feature pointers such as `mcp.source` and `bin.source` read repository-owned content outside conventional source homes.
...
`skillset create` puts its initial working guidance in `.skillset/rules/skillset-workflow.md`; the first confirmed build generates root `AGENTS.md` from that source alongside your project instructions.
...
Claude-enabled unscoped instructions aggregate into root `CLAUDE.md`. Instructions with `paths` remain under `.claude/rules/**/*.md` with native frontmatter, so Claude loads each instruction once. Agent Instructions owns the standard `AGENTS.md` files, and Codex may consume matching output. Standard destinations derive from instruction scope: `docs/**/*.md` writes `docs/AGENTS.md`, while broad globs such as `**/*.ts` scan matching repo files and use the lowest common directory. Rules sharing an aggregate destination are concatenated in source order with `<!-- source: ... -->` boundary comments and no source-only frontmatter. Codex truncates `AGENTS.md` beyond `project_doc_max_bytes` (32 KiB default); Skillset warns when generated output crosses it. Narrow the instruction scopes or adjust the provider's limit. Ordinary builds, including `--yes`, block unmanaged instruction collisions. Preserve existing guidance and move or adopt it into canonical source through a reviewed workflow before asking Skillset to own the destination; provider opt-outs cannot suppress the inherent `AGENTS.md` baseline.
