...
| Architecture | Tokenizer extracts `$<alias>` not `w/<alias>` |
...
| Key Types | `InvocationToken` - Parsed `$<alias>` token |
...
skillset show <ref>           # Show skill metadata
skillset load <ref>           # Load skill content
