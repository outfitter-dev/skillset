...
The optional Lefthook setup mirrors the repo's local review gates, and `lefthook.yml` is their single source of truth. Pre-commit checks staged whitespace and, when Skillset source or generated outputs are staged, runs the self-hosted lint/check pair. Pre-push fails fast on cheap gates first — whitespace across the pushed range (`git diff --check origin/main...HEAD`) and workflow lint when `actionlint` is available — then runs the two heavy gates in parallel: `bun run check` (the same aggregate CI runs) and `skillset ci --since origin/main` with a report under `.skillset/build/`. Both hooks can be run anywhere via `bun run hooks:pre-commit` / `bun run hooks:pre-push`.
