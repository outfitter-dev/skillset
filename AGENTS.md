...
`bun run change-stream:guard` keeps the append-only `.skillset/changes/*.jsonl` streams safe to merge. `.gitattributes` declares the built-in `merge=union` strategy for them so cross-branch merges and restacks keep both sides' appended records instead of conflicting; the guard checks what union cannot — trailing newline, valid JSONL objects, unique record ids, the declared `merge=union` attribute, and that trunk's stream is an in-order prefix of the working copy. Record order is load-bearing because release/pending state folds events in file order (ADR-0038). The guard reads each stream at `git merge-base HEAD $(scripts/git-trunk.sh)` and fails at the first line where the working copy stops matching it, so trunk's records may never be reordered, edited, or removed; new records only go after them. Event `createdAt`/`appliedAt`/`amendedAt` is event time, not sequence, so an appended block older than trunk's tail passes and there is no allowlist. Never sort a stream to fix a failure; restore trunk's records exactly and move the branch's records after them.
