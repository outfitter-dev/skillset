...
`bun run git-env:guard` fails when a production or test spawn passes `"git"` in argv without the shared sanitized environment (`gitSafeEnv`, `gitReadOnlyEnv`, or `testGitEnv`). Hook-exported `GIT_DIR` overrides `git -C` and cwd discovery; the helper lives in `@skillset/core` and is the only definition.
