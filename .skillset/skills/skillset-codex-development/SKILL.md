...
5. Run `bun run skillset:check` for current source authoring diagnostics and `bun run skillset:verify` for generated-output freshness when the change touches Skillset source, generated output, or docs/guidance that explain the command boundary.
6. Run `bun run check` before handoff.
7. Report generated file counts and any skipped checks explicitly.

## Safety Checks

- Configured generated destination roots must stay inside the repo, outside `.skillset/`, `.skillset/cache/`, and `.skillset/snapshots/`, and unique per active target output. Skillset-owned operational cache paths are reported under `.skillset/cache/` but physically resolve to the repo's XDG cache bucket; Git-backed recovery snapshots may live under `.skillset/snapshots/`.
