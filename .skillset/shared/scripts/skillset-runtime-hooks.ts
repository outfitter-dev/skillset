...
export async function hasSkillsetRuntimeSourceChanges(rootPath = process.cwd()): Promise<boolean> {
  const result = await capture(["git", "status", "--porcelain=v1", "--untracked-files=all", "--", ...SOURCE_CHANGE_PATHS], {
    cwd: rootPath,
  });
...
export async function runRuntimeHook(command: RuntimeHookCommand, rootPath = process.cwd()): Promise<number> {
  if (!(await hasSkillsetRuntimeSourceChanges(rootPath))) return 0;

  if (command === "post-tool-use") {
    await runSkillset(["change", "status", "--root", "."], { allowFailure: true, rootPath });
    return 0;
  }

  const changeCheck = await runSkillset(["change", "check", "--root", "."], { allowFailure: false, rootPath });
  if (changeCheck !== 0) return changeCheck;
  return runSkillset(["check", "--root", "."], { allowFailure: false, rootPath });
}
...
async function runShell(command: string, args: readonly string[], cwd: string): Promise<number> {
  const proc = Bun.spawn({
...
function parseRuntimeHookCommand(value: string | undefined): RuntimeHookCommand {
  if (value === "post-tool-use" || value === "stop") return value;
  throw new Error("usage: bun .skillset/shared/scripts/skillset-runtime-hooks.ts <post-tool-use|stop>");
}
