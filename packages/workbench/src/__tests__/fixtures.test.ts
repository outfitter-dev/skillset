...
describe("workbench fixtures", () => {
  ...
  test("invalid fixture reports deterministic source, resource, and runtime diagnostics", async () => {
    ...
    expect(diagnostics.diagnostics.map(formatWorkbenchDiagnostic)).toEqual([
      ...
      "skillset.yaml:1: error: schema/workspace-config: duplicate compile target claude",
      "skillset.yaml:1: error: schema/workspace-config: unsupported compile feature key surprise",
      "skillset.yaml:1: error: schema/workspace-config: unsupported compile skillset key surprise",
