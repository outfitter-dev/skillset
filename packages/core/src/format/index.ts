...
function resolveOutputOptions(
  ...
  // Prefer options from ResolveResult (propagated from resolver)
  // Fall back to SkillEntry (found in config lookup)
  // Finally fall back to global config defaults
  const includeLayout =
  ...
  for (const entry of resolved) {
    ...
    lines.push(
      ...
      await formatSkillBlock(
        ...
        entry.skill.skillRef,
        undefined, // no ResolveResult for set skills
        entryConfig
      )
    );
  }
