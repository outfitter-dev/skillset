...
`,
  ...
  it.each([
    ["render results", (lock: Record<string, unknown>) => {
      lock.renderResults = [];
    }],
    ["target", (lock: Record<string, unknown>) => {
      lock.target = "claude";
    }],
    ["feature metadata", (lock: Record<string, unknown>) => {
      const features = lock.features as Record<string, unknown>;
      features.promptArguments = !features.promptArguments;
    }],
    ["provenance hash", (lock: Record<string, unknown>) => {
      lock.provenanceHash = "sha256:invalid";
    }],
  ] as const)("classifies edited %s as output divergence and backs it up", async (_label, editLock) => {
    const root = await fixture({
      "skillset.yaml": `
skillset:
  name: edited-lock-metadata-root
claude: false
codex: true
cursor: false
`,
    ...
    const lock = JSON.parse(await readFile(lockPath, "utf8")) as Record<
      ...
      unknown
    >;
    expect(lock.provenanceHash).toMatch(/^sha256:[a-f0-9]{64}$/);
    editLock(lock);
    await writeFile(lockPath, `${JSON.stringify(lock, null, 2)}\n`, "utf8");
  ...
  it.each([
    ...
    ["with metadata and item edits", (lock: Record<string, unknown>) => {
      lock.target = "claude";
      const items = lock.items as Array<Record<string, unknown>>;
      items[0]!.sourceHash = `sha256:${"0".repeat(64)}`;
    }],
  ] as const)("repairs a missing v2 provenance hash %s with a backup", async (_label, editLock) => {
    const root = await fixture({
      "skillset.yaml": `
skillset:
  name: legacy-v2-lock-root
claude: false
codex: true
cursor: false
`,
...
`,
    ...
    await buildSkillsetResult(root);
    const lockPath = join(root, "skillset.lock");
    const lock = await downgradeLockToV1(root, "skillset.lock");
    editLock(lock);
    await writeFile(lockPath, `${JSON.stringify(lock, null, 2)}\n`, "utf8");
  ...
  it.each([
    ...
    const root = await fixture({
      "skillset.yaml": `
skillset:
  name: mixed-edited-legacy-lock-root
claude: false
codex: true
cursor: false
`,
  ...
  it("keeps schema-v1 migration source changes source-driven", async () => {
    const root = await fixture({
      "skillset.yaml": `
skillset:
  name: legacy-source-change-root
claude: false
codex: true
cursor: false
`,
    ...
    await buildSkillsetResult(root);
    await downgradeLockToV1(root, "skillset.lock");
    await writeFile(
      join(root, ".skillset/rules/root.md"),
      "# Updated project instructions\n",
      "utf8"
...
async function downgradeLockToV1(
  root: string,
  ...
  const lock = JSON.parse(await readFile(lockPath, "utf8")) as Record<
    ...
    unknown
  >;
  const outputRoot = lock.outputRoot as string;
  const items = lock.items as Array<Record<string, unknown>>;
  lock.schemaVersion = 1;
  delete lock.provenanceHash;
  for (const item of items) {
    ...
    for (const file of item.files as string[]) {
      hash.update(file);
      hash.update("\0");
      hash.update(await readFile(join(root, outputRoot, file)));
      hash.update("\0");
    }
