  ...
  for (const [name, server] of Object.entries(model.servers)) {
    ...
    for (const value of [server.command, ...(server.args ?? []), server.cwd]) {
      ...
      if (supportPath !== undefined) {
        collectReferencedSupportPath(supportPath, supportPaths);
      }
    }
  }
...
function collectReferencedSupportPath(
  ...
  supportPaths.add(posix.normalize(value));
}

function pluginRootSupportPath(value: string): string | undefined {
  if (value.startsWith("./")) return value.slice(2);
  if (value.startsWith("${PLUGIN_ROOT}/")) {
    return value.slice("${PLUGIN_ROOT}/".length);
  }
  return undefined;
}
