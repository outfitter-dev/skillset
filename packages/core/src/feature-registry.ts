...
export const skillsetFeatureRegistry = defineFeatureRegistry([
  ...
  feature({
    ...
    evidence: [
      ...
      test("packages/core/src/__tests__/render-result.test.ts", "lowering outcome schema validation coverage"),
      test("packages/core/src/__tests__/render-result-build.test.ts", "build lowering outcomes cover emitted, target-native, transformed, unsupported, policy-gated, scoped, and status-matrix outcomes"),
    ...
    kind: "workflow",
    loweringOwner: "packages/core/src/render-result-collector.ts",
