import { parseMarkdown, parseYamlRecord, stringifyJson, stringifyMarkdown, stringifyYaml } from "./yaml";
...
function writeTomlRecord(lines: string[], path: readonly string[], value: JsonRecord): void {
  ...
  if (path.length > 0) lines.push(`[${path.map(quoteTomlKey).join(".")}]`);
  lines.push(...scalars);
...
function stringifyTomlValue(value: unknown): string {
  ...
  throw new Error("TOML values must be strings, booleans, numbers, arrays, or tables");
}
