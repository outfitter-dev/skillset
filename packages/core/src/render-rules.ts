...
const CLAUDE_PROJECT_INSTRUCTIONS = "CLAUDE.md";
...
export async function renderRules(
  ...
  rendered.push(...(await renderClaudeInstructions(graph, lockRoots)));
  rendered.push(...(await renderAgentInstructionFiles(graph, lockRoots)));
  rendered.push(...(await renderCursorRules(graph, lockRoots)));
  return rendered;
}

async function renderClaudeInstructions(
  graph: BuildGraph,
  lockRoots: Map<string, LockRoot>
): Promise<readonly RenderedFile[]> {
  const rendered: RenderedFile[] = [];
  const claudeRules = graph.rules.filter(
    (sourceRule) => sourceRule.targets.claude.enabled
  );
  const unscopedRules = claudeRules.filter(
    (sourceRule) => readRulePaths(sourceRule).length === 0
  );

  if (unscopedRules.length > 0) {
    const markdown = await renderClaudeProjectMarkdown(
      graph,
      unscopedRules,
    ...
    lockRootsFor(lockRoots, WORKSPACE_LOCK_ROOT, "workspace").items.push(
      lockItemForRule({
        ...
        files: [file],
        graph,
        name: CLAUDE_PROJECT_INSTRUCTIONS,
        owner: { target: "claude" },
        outputRoot: WORKSPACE_LOCK_ROOT,
        outputPath: CLAUDE_PROJECT_INSTRUCTIONS,
  ...
  for (const rule of claudeRules.filter(
    (sourceRule) => readRulePaths(sourceRule).length > 0
  )) {
...
async function renderClaudeProjectMarkdown(
  graph: BuildGraph,
...
async function renderCodexAgentsMarkdown(
  ...
  // their Claude projection stays native in root CLAUDE.md or .claude/rules.
