...
async function renderClaudeRules(
  ...
  for (const rule of graph.rules.filter(
    (sourceRule) => sourceRule.targets.claude.enabled && !sourceRule.rootFrontPage
  )) {
...
async function renderAgentInstructionFiles(
  ...
  for (const projection of projections) {
    ...
    const rootFrontPage = rules.find((rule) => rule.rootFrontPage);
    const sourcePath =
      rootFrontPage === undefined
        ? workspaceRelativeSourcePath(graph, graph.rulesDir)
        : relative(graph.rootPath, rootFrontPage.sourcePath);
    const file = {
  ...
  for (const [destination, item] of lockItems) {
    const rootFrontPage = item.rules.find((rule) => rule.rootFrontPage);
    const sourcePath =
      rootFrontPage === undefined
        ? workspaceRelativeSourcePath(graph, graph.rulesDir)
        : relative(graph.rootPath, rootFrontPage.sourcePath);
...
async function renderCursorRules(
  ...
  for (const rule of graph.rules.filter(
    (sourceRule) => sourceRule.targets.cursor.enabled && !sourceRule.rootFrontPage
  )) {
...
async function renderCodexAgentsMarkdown(
  ...
  const renderedSections = resolvedSections
    ...
    .map(
      (section) =>
        section.rule.rootFrontPage
          ? section.body
          : `<!-- source: ${relative(graph.rootPath, section.rule.sourcePath)} -->\n${section.body}`
    );
  const hasRootFrontPage = resolvedSections.some(
    (section) => section.rule.rootFrontPage && section.body.length > 0
  );
  const content = [
    ...(hasRootFrontPage
      ? []
      : [
          `<!-- Generated by ${GENERATED_BY} from ${workspaceRelativeSourcePath(graph, graph.rulesDir)}. Do not edit directly. -->`,
          "",
        ]),
