...
export async function loadBuildGraph(
  ...
  const rootConfig = parseYamlRecord(
  ...
  const rootTargets = resolveTargets(
    readCompileTargets(rootConfig, rootConfigPath),
    rootConfig,
    rootConfigPath,
    {
      allowDefaults: true,
  ...
  const filteredTargets = applyTargetFilter(
    rootTargets,
  ...
  await validateSupports(rootConfig.supports, {
    label: rootConfigPath,
    rootPath,
    warnings,
  });
  ...
  const standaloneSkills = await loadStandaloneSkills(
    rootPath,
    sourceDir,
    filteredTargets,
    warnings
  ...
  if (
    ...
    projectIslands.length === 0
  ) {
    throw new Error(
      `skillset: no source plugins, skills, instructions, project agents, or target-native islands found under ${sourceDir}/`
    );
  }

  const outputRoots = await outputRootsFor(
    rootPath,
    outputs,
    plugins,
    standaloneSkills,
...
function applyTargetFilter(
  ...
  for (const target of enabledTargets) {
    if (!targets[target].enabled) {
      throw new Error(
        `skillset: test target ${target} is not enabled by ${label} target configuration`
      );
    }
  }
  return {
    claude: enabledTargets.has('claude')
      ? targets.claude
      : { enabled: false, options: {} },
    codex: enabledTargets.has('codex')
      ? targets.codex
      : { enabled: false, options: {} },
  };
}
...
async function loadProjectIslands(
  ...
  const srcPath = resolveInside(
    rootPath,
    join(sourceDir, TARGET_NATIVE_SOURCE_DIR)
  );
  ...
  islands.push(
    ...(await loadTargetIsland(rootPath, join(srcPath, 'claude'), 'claude'))
  );
  islands.push(
    ...(await loadTargetIsland(rootPath, join(srcPath, 'codex'), 'codex'))
  );
  ...
  if (await exists(pluginsPath)) {
    ...
    for (const plugin of plugins) {
      ...
      islands.push(
        ...(await loadTargetIsland(
          rootPath,
          join(pluginPath, 'claude'),
          'claude',
          plugin.id
        ))
      );
  ...
  for (const island of islands) {
    if (
      island.target === 'codex' &&
      island.relativePath.endsWith('.rules') &&
    ...
    if (
      island.target === 'codex' &&
      island.relativePath.endsWith('.rules') &&
      !island.relativePath.startsWith('rules/')
    ) {
      throw new Error(
  ...
  return islands.toSorted((left, right) =>
    compareStrings(
      `${left.plugin ?? ''}/${left.target}/${left.relativePath}`,
      `${right.plugin ?? ''}/${right.target}/${right.relativePath}`
    )
  );
}
...
async function loadProjectAgents(
  ...
  const agentsPath = resolveInside(
    rootPath,
    join(sourceDir, TARGET_NATIVE_SOURCE_DIR, PROJECT_AGENTS_DIR)
  );
...
function rejectUnsupportedPortableFrontmatter(
...
function sanitizeProjectAgentName(name: string, sourcePath: string): string {
  ...
  if (outputName.length === 0) {
    throw new Error(
      `skillset: ${sourcePath} project agent name must contain a letter or number`
    );
  }
  return validateSlug(outputName, `project agent output name in ${sourcePath}`);
}

function validateProjectAgentCollisions(
  agents: readonly SourceProjectAgent[]
): void {
  ...
  for (const agent of agents) {
    for (const target of targetNames()) {
      if (!agent.targets[target].enabled) {
        continue;
      }
      ...
      const targetName =
        readString(agent.targets[target].options, 'name') ?? agent.name;
...
async function loadInstructions(
  ...
  const canonicalPath = resolveInside(
    rootPath,
    join(sourceDir, INSTRUCTIONS_DIR)
  );
  const canonicalFiles = (await exists(canonicalPath))
    ? await findMarkdownFiles(canonicalPath)
    : [];
  const rulesPath = resolveInside(rootPath, join(sourceDir, 'rules'));
  const rulesFiles = (await exists(rulesPath))
    ? await findMarkdownFiles(rulesPath)
    : [];
  if (rulesFiles.length > 0) {
    throw new Error(
      `skillset: ${sourceDir}/rules is not supported; move instruction files to ${sourceDir}/${INSTRUCTIONS_DIR}`
    );
  }
  if (canonicalFiles.length === 0) {
    return { instructionsDir: INSTRUCTIONS_DIR, rules: [] };
  }
  ...
  for (const sourcePath of ruleFiles) {
    ...
    await validateSupports(frontmatter.supports, {
      label: relative(rootPath, sourcePath),
      rootPath,
      warnings,
    });
    const targets = resolveFeatureTargets(
  ...
  return {
    ...
    rules: rules.toSorted((left, right) =>
      compareStrings(left.relativePath, right.relativePath)
    ),
  };
}

function normalizeRuleFrontmatter(
  frontmatter: SourceRule['frontmatter'],
  label: string
): SourceRule['frontmatter'] {
...
async function loadPlugins(
  ...
  for (const entry of entries.toSorted((left, right) =>
    ...
    plugins.push(
      await loadPlugin(rootPath, sourceDir, id, rootTargets, warnings, outputs)
    );
  }
...
async function loadPlugin(
  ...
  validateConfigDocument(config, configPath, {
    featureKeys: PLUGIN_FEATURE_KEYS,
  });
  await validateSupports(config.supports, {
    label: relative(rootPath, configPath),
    rootPath,
    warnings,
  });
  const dependencies = readPluginDependencies(
    config.dependencies,
    relative(rootPath, configPath)
  );
  ...
  if (
    ...
    outputIncludes(codexPluginSelection, id) &&
    (await exists(join(pluginPath, 'agents')))
  ) {
    throw new Error(
...
async function loadPluginFeatures(
  ...
  return features.toSorted((left, right) =>
...
async function loadPluginFeature(
  ...
  if (raw === undefined) {
    ...
    if (!hasConventionalSource) {
      throw new Error(
        `skillset: plugin ${pluginId} feature ${key}: true requires conventional source ${relative(rootPath, conventionalSource)}`
      );
    }
    sourcePath = conventionalSource;
    ...
    if (sourcePointer === undefined) {
      throw new Error(
        `skillset: plugin ${pluginId} feature ${key} requires source`
      );
    }
    sourcePath = await resolveRepoSourcePointer(
      rootPath,
      sourcePointer,
      `${configPath}.${key}.source`,
      outputRoots
    );
    origin = 'explicit';
  } else {
    throw new Error(
      `skillset: expected ${configPath}.${key} to be true, false, or an object`
    );
  }
  ...
  if (
    key === 'bin' &&
    targets.codex.enabled &&
    outputIncludes(codexPluginSelection, pluginId)
  ) {
    throw new Error(
  ...
  if (key === 'mcp' && !stats.isFile()) {
    throw new Error(
      `skillset: plugin ${pluginId} feature mcp source must be a file`
    );
  }
  if (key === 'bin' && !stats.isDirectory()) {
    throw new Error(
      `skillset: plugin ${pluginId} feature bin source must be a directory`
    );
  }
...
async function resolveRepoSourcePointer(
  ...
  const sourcePath = resolveInside(
    rootPath,
    sourcePointer.slice('repo:'.length)
  );
  const outputRoot = outputRoots.find((root) =>
    isInsidePath(sourcePath, resolveInside(rootPath, root.path))
  );
  ...
  if (!(await exists(sourcePath))) {
    throw new Error(
      `skillset: ${label} points to missing path ${sourcePointer}`
    );
  }
  return sourcePath;
}
...
async function resolvePluginConfigPath(pluginPath: string): Promise<string> {
  ...
  if (candidates.length === 0) {
    throw new Error(
      `skillset: expected plugin config skillset.yaml in ${pluginPath}`
    );
  }
  if (candidates.length > 1) {
    throw new Error(
      `skillset: plugin ${pluginPath} has both skillset.yaml and config.yaml`
    );
  }
...
async function loadSkillsFromDirectory(
  ...
  for (const sourcePath of skillFiles) {
    ...
    if (metadata.name !== undefined) {
      throw new Error(
        `skillset: ${sourcePath} uses unsupported skillset.name; use top-level name`
      );
    }
    if (metadata.id !== undefined) {
      throw new Error(
        `skillset: ${sourcePath} uses unsupported skillset.id; use top-level name`
      );
    }
    if (metadata.version !== undefined) {
      throw new Error(
        `skillset: ${sourcePath} uses unsupported skillset.version; use top-level version`
      );
    }
    ...
    const resources = await readSkillResources(parts.frontmatter.resources, {
      ...
      ...(pluginPath === undefined
        ? {}
        : { pluginSharedPath: join(pluginPath, 'shared') }),
...
function warnPortableModel(
  ...
  const missingTargets = targetNames().filter(
    (target) =>
      ...
      readString(targets[target].options, 'model') === undefined
  );
...
async function outputRootsFor(
  ...
  const roots = new Map(
    activeRoots.map((outputRoot) => [outputRoot.path, outputRoot])
  );
  ...
  for (const outputRoot of configuredOutputRoots(outputs)) {
    ...
    if (
      await exists(
        join(resolveInside(rootPath, outputRoot.path), '.skillset.lock')
      )
    ) {
  ...
  return [...roots.values()].toSorted((left, right) =>
...
function activeOutputRoots(
  ...
  if (
    plugins.some(
      (plugin) =>
        plugin.targets.claude.enabled &&
        outputIncludes(outputs.targetOutputs.claude.plugins, plugin.id)
    )
  ) {
    roots.push({
      label: 'outputs.plugins.claude',
      path: outputs.plugins.claude,
    });
  }
  if (
    plugins.some(
      (plugin) =>
        ...
        outputIncludes(outputs.targetOutputs.codex.plugins, plugin.id)
    )
  ) {
    roots.push({ label: 'outputs.plugins.codex', path: outputs.plugins.codex });
  }
  if (
    standaloneSkills.some(
      (skill) =>
        skill.targets.claude.enabled &&
        outputIncludes(outputs.targetOutputs.claude.skills, skill.id)
    )
  ) {
  ...
  if (
    standaloneSkills.some(
      (skill) =>
        ...
        outputIncludes(outputs.targetOutputs.codex.skills, skill.id)
    )
  ) {
    roots.push({ label: 'outputs.skills.codex', path: outputs.skills.codex });
  }
  return roots.toSorted((left, right) => compareStrings(left.path, right.path));
}
...
function validateProjectRoots(
  ...
  for (const target of targetNames()) {
    const hasProjectAgentOutput = projectAgents.some(
      (agent) => agent.targets[target].enabled
    );
    const hasProjectIslandOutput = projectIslands.some(
      (island) => island.plugin === undefined && island.target === target
    );
    if (!hasProjectAgentOutput && !hasProjectIslandOutput) {
      continue;
    }
    const projectRoot = {
    ...
    const absoluteProjectRoot = validateOutputRootNotInsideSource(
      rootPath,
      sourcePath,
      projectRoot
    );
    const targetProjectAgents = projectAgents.filter(
      (agent) => agent.targets[target].enabled
    );
    const targetProjectIslands = projectIslands.filter(
      (island) => island.plugin === undefined && island.target === target
    );
    for (const outputRoot of outputRoots) {
      ...
      const overlappingAgent = targetProjectAgents.find((agent) =>
        isSameOrInside(
          resolveInside(
            rootPath,
            projectAgentOutputPath(projectRoot.path, target, agent)
          ),
          absoluteOutputRoot
        )
      );
      ...
      const overlappingIsland = targetProjectIslands.find((island) =>
        isSameOrInside(
          resolveInside(rootPath, join(projectRoot.path, island.relativePath)),
          absoluteOutputRoot
        )
      );
      if (overlappingIsland === undefined) {
        continue;
      }
      throw new Error(
...
function targetProjectRoot(
  rootTargets: BuildGraph['root']['targets'],
  target: 'claude' | 'codex'
): string {
  return (
    readString(rootTargets[target].options, 'projectRoot') ??
    (target === 'claude' ? '.claude' : '.codex')
  );
}

function projectAgentOutputPath(
  projectRoot: string,
  target: 'claude' | 'codex',
  ...
  return join(
    projectRoot,
    'agents',
    `${agent.outputName}.${target === 'claude' ? 'md' : 'toml'}`
  );
}
...
function validateOutputRoots(
  ...
  for (const outputRoot of outputRoots) {
    const absoluteOutputRoot = validateOutputRootNotInsideSource(
      rootPath,
      sourcePath,
      outputRoot
    );
