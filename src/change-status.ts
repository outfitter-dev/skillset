...
import { diffSkillset } from './build';
import type { SkillsetDiff } from './build';
...
export async function changeStatus(
  ...
  const stagedSnapshot =
    options.staged === true ? await snapshotGitIndex(rootPath) : undefined;
  try {
    const current = await collectSourceInventory(
      stagedSnapshot ?? rootPath,
      options
    );
    const baseline = await resolveBaselineInventory(rootPath, options);
    const generatedDrift = await diffSkillset(
      stagedSnapshot ?? rootPath,
      options
    );
    ...
    if (stagedSnapshot !== undefined) {
      await rm(stagedSnapshot, { force: true, recursive: true });
    }
  }
}
...
async function sourceInventoryForGraph(
  graph: BuildGraph
): Promise<SourceInventory> {
  ...
  return {
    hashSchema: SOURCE_HASH_SCHEMA,
    units: [...dedupeUnits(units)].toSorted((left, right) =>
...
async function skillUnit(
  ...
  const preprocessDependencies = await skillPreprocessDependencies(
    graph,
    skill,
    plugin
  );
  const sourcePaths = await sourcePathsForSkill(
    graph,
    sourceDir,
    skill.resources,
...
async function projectAgentUnit(
  graph: BuildGraph,
  agent: SourceProjectAgent
): Promise<SourceUnit> {
  const preprocessDependencies = await projectAgentPreprocessDependencies(
    graph,
    agent
  );
...
async function islandUnit(
  ...
  const preprocessDependencies = await islandPreprocessDependencies(
    graph,
    island
  );
  const owner: 'project' | `plugin:${string}` =
    island.plugin === undefined ? 'project' : `plugin:${island.plugin}`;
  const id = selectorForTargetNativeIsland(
    island.target,
    owner,
    island.relativePath
  );
...
async function pluginCompanionUnits(
  ...
  const featureSourcePaths = new Set(
    plugin.features.map((feature) => feature.sourcePath)
  );
  const units: SourceUnit[] = [];
  for (const companionPath of PLUGIN_COMPANION_PATHS) {
    const sourcePath = join(plugin.path, companionPath);
    if (featureSourcePaths.has(sourcePath) || !(await exists(sourcePath))) {
      continue;
    }
    units.push(
      await fileUnit(
...
function pluginAggregateUnit(
  ...
  hash.update(
    stringifyJson({
      claude: plugin.targets.claude.options,
      codex: plugin.targets.codex.options,
    })
  );
...
async function sourcePathsForSkill(
  ...
  return sortedUnique([
    ...(await sourcePathsForPath(
      graph,
      sourceDir,
      isGeneratedEntityChangelogPath
    )),
...
async function sourcePathsForPath(
  ...
  if (stats.isFile()) {
    return [relativePath(graph, sourcePath)];
  }
...
async function hashPreprocessDependencies(
  ...
  for (const dependency of [...dependencies].toSorted(compareStrings)) {
...
async function skillPreprocessDependencies(
  ...
  return [...dependencies].toSorted(compareStrings);
}
...
async function projectAgentPreprocessDependencies(
  ...
  await collect(
    readString(agent.targets.codex.options, 'developer_instructions')
  );
...
async function collectFiles(root: string): Promise<readonly string[]> {
  const entries = await Array.fromAsync(
    new Bun.Glob('**/*').scan({ cwd: root, dot: true, onlyFiles: true })
  );
  return entries
...
function compareInventories(
  ...
  return changes.toSorted((left, right) => compareStrings(left.id, right.id));
}
...
async function resolveBaselineInventory(
  ...
  const releaseInventory = await sourceInventoryFromReleaseState(
    rootPath,
    options,
    fallback.inventory
  );
...
async function sourceInventoryFromReleaseState(
  ...
  const releaseScopes = Object.entries(state.scopes).filter(
    ([, scope]) => scope.sourceHash !== undefined || scope.removed === true
  );
  ...
  for (const [id, scope] of releaseScopes) {
    ...
    const template =
      currentUnits.get(selector) ??
      fallbackUnits.get(selector) ??
      inferredReleaseUnit(selector, scope.sourceHash ?? '');
    units.set(selector, {
      ...template,
  ...
  return {
    ...
    inventory: {
      hashSchema: SOURCE_HASH_SCHEMA,
      units: [...units.values()].toSorted((left, right) =>
        compareStrings(left.id, right.id)
      ),
    },
  };
...
function kindForSourceUnitId(id: string): SourceUnitKind {
  ...
  if (/^plugin\.[^.]+\.companion:/.test(selector)) {
    return 'plugin-companion';
  }
  if (/^plugin\.[^.]+\.config:/.test(selector)) {
    return 'plugin-config';
  }
  if (/^plugin\.[^.]+\.feature:/.test(selector)) {
    return 'plugin-feature';
  }
  if (/^plugin\.[^.]+\.skill:/.test(selector)) {
    return 'plugin-skill';
  }
  if (/^(?:plugin\.[^.]+\.)?(?:claude|codex)\.[^.]+:/.test(selector)) {
...
async function sourceInventoryFromLock(
  ...
  let parsed: unknown;
  try {
    parsed = JSON.parse(await readFile(lockPath, 'utf-8')) as unknown;
  } catch {
    return undefined;
  }
  if (!isJsonRecord(parsed) || !isJsonRecord(parsed.sourceInventory)) {
    return undefined;
  }
  ...
  if (hashSchema === undefined || !Array.isArray(rawUnits)) {
    return undefined;
  }

  const units: SourceUnit[] = [];
  for (const rawUnit of rawUnits) {
    if (!isJsonRecord(rawUnit)) {
      continue;
    }
...
export async function snapshotGitIndex(rootPath: string): Promise<string> {
  ...
  await runCommand(
    ['git', '-C', rootPath, 'checkout-index', '--all', `--prefix=${tempRoot}/`],
    rootPath
  );
  return tempRoot;
}
...
async function defaultMergeBase(rootPath: string): Promise<string> {
  for (const candidate of ['origin/main', 'main']) {
    ...
    if (mergeBase !== undefined && mergeBase.trim().length > 0) {
  ...
  throw new Error(
    'skillset: could not resolve a change status baseline; pass --since <ref>'
  );
}
...
async function runCommand(
  ...
  throw new Error(
    `skillset: command failed: ${cmd.join(' ')}\n${stderr.trim()}`
  );
}

async function regionsForYaml(
  sourcePath: string
): Promise<readonly SourceUnitRegion[]> {
  try {
    return regionsForRecord(
      parseYamlRecord(await readFile(sourcePath, 'utf-8'), sourcePath)
    );
  } catch {
    return [];
  }
...
function regionsForRecord(record: JsonRecord): readonly SourceUnitRegion[] {
  ...
  if (record.supports !== undefined) {
    regions.push({ name: 'supports', severityBearing: false });
  }
  if (record.dependencies !== undefined) {
    regions.push({ name: 'dependencies', severityBearing: true });
  }
  ...
  if (record.mcp !== undefined) {
    regions.push({ name: 'mcp', severityBearing: true });
  }
...
function companionRegions(path: string): readonly SourceUnitRegion[] {
  ...
  if (path === '.app.json') {
    return [{ name: 'apps', severityBearing: true }];
  }
  if (path === 'commands') {
    return [{ name: 'commands', severityBearing: true }];
  }
  if (path === 'agents') {
    return [{ name: 'agents', severityBearing: true }];
  }
  if (path === 'scripts' || path === 'src') {
    return [{ name: path, severityBearing: true }];
  }
