...
const PLACEHOLDER_REASON_PATTERN =
  /^(todo|tbd|placeholder|none|n\/a|fill me in|write reason)$/i;
...
export async function changeCheck(
  ...
  try {
    ...
    const entries = await readPendingChangeEntries(
      stagedSnapshot ?? rootPath,
      options
    );
    return await validateChangeCheck(
      stagedSnapshot ?? rootPath,
      status,
      entries,
      options
...
async function validateChangeCheck(
  ...
  const currentById = new Map(
    status.sourceUnits.map((unit) => [unit.id, unit])
  );
  const changedById = new Map(
    status.sourceChanges.map((change) => [change.id, change])
  );
  ...
  const refEntries =
    options.ref === undefined
      ? entries
      : [resolvePendingChangeRef(entries, options.ref)];
  ...
  if (options.ref === undefined) {
    ...
    for (const entry of validEntries) {
      for (const scope of entry.scopes) {
        covered.add(scope);
        for (const impliedScope of impliedCoveredScopes(scope)) {
          covered.add(impliedScope);
        }
      }
    }
    for (const change of status.sourceChanges) {
      if (covered.has(change.id)) {
        continue;
      }
      issues.push({
  ...
  return {
    entries,
    issues: issues.toSorted(compareIssues),
...
function impliedCoveredScopes(scope: string): readonly string[] {
  ...
  return pluginScope === undefined || pluginScope === sourceUnitSelector(scope)
    ? []
    : [pluginScope];
}
...
export async function readPendingChangeEntries(
  ...
  const files = await Array.fromAsync(
    new Bun.Glob('*.md').scan({ cwd: pendingPath, onlyFiles: true })
  );
  ...
  for (const file of files.toSorted(compareStrings)) {
    ...
    const sourceHashes = readSourceHashEvidence(
      parts.frontmatter.evidence,
      scopes
    );
    entries.push({
      bump,
      ...
      hasInvalidGroup:
        parts.frontmatter.group !== undefined && group === undefined,
      id,
      ignored,
      path,
...
export function resolvePendingChangeRef(
  ...
  if (prefix.length < CHANGE_REF_MIN_LENGTH) {
    throw new Error(
      `skillset: expected change ref @${prefix} to use at least ${CHANGE_REF_MIN_LENGTH} hex characters`
    );
  }
...
function validatePendingEntry(
  ...
  if (entry.id === undefined) {
    issues.push(
      entryError(entry, 'change-id-missing', 'pending change entry requires id')
    );
    ...
    issues.push(
      entryError(
        entry,
        'change-id-duplicate',
        `pending change id ${entry.id} is used by multiple entries`
      )
    );
  }
    ...
    if (scopeClasses.size > 1) {
      issues.push(
        entryError(
          entry,
          'change-scope-mixed',
          'pending change entry must not mix repo/project scopes with user/global scopes'
        )
      );
    }
    for (const scope of entry.scopes) {
      if (!context.validScopeIds.has(scope)) {
        issues.push(
          entryError(
            ...
            `scope ${sourceUnitDisplay(scope)} does not match a known source unit`
          )
        );
      }
    }
  ...
  if (entry.reason.length < context.minReasonLength) {
    issues.push(
      entryError(
        ...
        `reason must be at least ${context.minReasonLength} characters`
      )
    );
  }
  ...
  if (entry.group !== undefined && entry.group.id.trim().length === 0) {
    issues.push(
      entryError(entry, 'change-group-invalid', 'group id must be non-empty')
    );
  }
  ...
  for (const scope of entry.scopes) {
    ...
    if (hashes.length === 0) {
      issues.push(
        entryError(
          ...
          `scope ${sourceUnitDisplay(scope)} requires source hash evidence`
        )
      );
      continue;
    }
    if (!hashes.includes(expectedHash)) {
      issues.push(
        entryError(
          entry,
          'change-evidence-stale',
          `scope ${sourceUnitDisplay(scope)} source hash evidence is stale`
        )
      );
    }
  }
...
function hasSeverityBearingRegion(
  regions: readonly { readonly severityBearing: boolean }[] | undefined
): boolean {
...
function readScopes(frontmatter: JsonRecord): readonly string[] {
  ...
  const { scopes } = frontmatter;
  const values: string[] = [];
  if (scope !== undefined) {
    values.push(...readScopeValue(scope));
  }
  if (scopes !== undefined) {
    values.push(...readScopeValue(scopes));
  }
  return [
    ...new Set(
      values
        .map((value) => sourceUnitSelector(value.trim()))
...
function readEvidenceHash(record: JsonRecord): string | undefined {
  return (
    ...
    readString(record, 'sourceHash') ??
    readString(record, 'currentHash')
  );
}

async function readReasonMinLength(
  rootPath: string,
  options: ChangeStatusOptions
): Promise<number> {
  const sourceDir = options.sourceDir ?? '.skillset';
  ...
  if (!isJsonRecord(reason)) {
    return DEFAULT_REASON_MIN_LENGTH;
  }
  const { minLength } = reason;
...
function scopeClass(scope: string): 'repo' | 'user' {
  const normalizedScope = sourceUnitSelector(scope);
  return normalizedScope.startsWith('user:') ||
    normalizedScope.startsWith('global:')
    ? 'user'
    : 'repo';
}

function compareIssues(
  left: ChangeCheckIssue,
  right: ChangeCheckIssue
): number {
