...
export async function readReleaseState(
  ...
  for (const [scope, value] of Object.entries(rawScopes).toSorted(
    ([left], [right]) => compareStrings(left, right)
  )) {
    if (!isJsonRecord(value)) {
      throw new Error(
        `skillset: release state scope ${scope} must be an object`
      );
    }
    const version = readString(value, 'version');
    if (version === undefined) {
      throw new Error(
        `skillset: release state scope ${scope} requires version`
      );
    }
    ...
    if (removed !== undefined && typeof removed !== 'boolean') {
      throw new Error(
        `skillset: release state scope ${scope} removed must be a boolean`
      );
    }
    const sourceHash = readString(value, 'sourceHash');
    if (sourceHash !== undefined && !/^sha256:[0-9a-f]{64}$/.test(sourceHash)) {
      throw new Error(
        `skillset: release state scope ${scope} sourceHash must be a sha256 digest`
      );
    }
...
export async function writeReleaseState(
  ...
  await writeFile(
    absolutePath,
    stringifyJson({ schemaVersion: 1, scopes }),
    'utf-8'
  );
  return relativePath;
}
...
function releaseStatePath(rootPath: string, options: SkillsetOptions): string {
  return resolveInside(
    rootPath,
    join(options.sourceDir ?? '.skillset', STATE_FILE)
  );
}
