...
function readPackageSupports(
  value: JsonValue,
  label: string
): readonly PackageSupport[] {
  if (typeof value === 'string') {
    return [parseCompactSupport(value, label)];
  }
  if (Array.isArray(value)) {
    return value.map((item) => readPackageSupport(item, label));
  }
  if (!isJsonRecord(value)) {
    throw new Error(
      `skillset: expected ${label}.supports to be a string, array, or object`
    );
  }
  ...
  for (const key of Object.keys(value)) {
    if (key !== 'packages') {
      throw new Error(
        `skillset: unsupported ${label}.supports key ${key}; v1 supports packages`
      );
    }
  }
  if (packages === undefined) {
    throw new Error(
      `skillset: expected ${label}.supports.packages to be an array`
    );
  }
  ...
  return packages.map((item) =>
    readPackageSupport(item, `${label}.supports.packages`)
  );
}
...
function readPackageSupport(value: JsonValue, label: string): PackageSupport {
  ...
  if (!isJsonRecord(value)) {
    throw new Error(
      `skillset: expected ${label} support entries to be strings or objects`
    );
  }
  ...
  if (name === undefined) {
    throw new Error(
      `skillset: expected ${label} support entry to include name`
    );
  }
  if (range === undefined) {
    throw new Error(
      `skillset: expected ${label} support entry ${name} to include range`
    );
  }
...
function parseCompactSupport(value: string, label: string): PackageSupport {
  ...
  if (name === undefined || name.length === 0 || range.length === 0) {
    throw new Error(
      `skillset: expected ${label} compact support to look like "<package> <range>"`
    );
  }
  return { name, onMismatch: 'warn', range };
}
...
async function checkPackageSource(
  ...
  if (support.source === undefined) {
    return;
  }
  if (!support.source.startsWith('repo:')) {
    throw new Error(
      `skillset: ${context.label} supports ${support.name} source must use repo:<path>`
    );
  }
  ...
  if (relativeSource.trim().length === 0) {
    throw new Error(
      `skillset: ${context.label} supports ${support.name} source must include a repo path`
    );
  }
  ...
  let parsed: unknown;
  try {
    parsed = JSON.parse(await readFile(sourcePath, 'utf-8')) as unknown;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    warnOrThrow(
      support,
      context,
      `${support.source} could not be read as package JSON: ${message}`
    );
    return;
  }
  if (!isJsonRecord(parsed)) {
    warnOrThrow(
      support,
      context,
      `${support.source} is not a package JSON object`
    );
    return;
  }
...
export function validateSemverRange(range: string, label: string): void {
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `skillset: invalid ${label} range ${JSON.stringify(range)}: ${message}`,
      { cause: error }
    );
  }
}
...
function satisfiesRange(version: string, range: string): boolean {
  ...
  return rangeComparators(range, 'range').every((comparator) =>
    comparator(parsedVersion)
  );
}
...
function rangeComparators(range: string, label: string): readonly Comparator[] {
  ...
  if (trimmed.includes('||')) {
    throw new Error('OR ranges are not supported in v1');
  }

  const comparators = trimmed
    .split(/\s+/)
    .filter((token) => token.length > 0)
    .map((token) => comparatorForToken(token, label));
...
function comparatorForToken(token: string, label: string): Comparator {
  if (token.startsWith('^')) {
    return caretComparator(parseVersion(token.slice(1), label));
  }
  if (token.startsWith('~')) {
    return tildeComparator(parseVersion(token.slice(1), label));
  }

  const match = token.match(COMPARATOR_PATTERN);
  if (match === null) {
    throw new Error(`unsupported comparator ${token}`);
  }
...
function caretComparator(version: Semver): Comparator {
  ...
  return (actual) =>
    compareSemver(actual, version) >= 0 && compareSemver(actual, upper) < 0;
}
