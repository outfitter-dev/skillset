...
export function readAllowedTools(
  ...
  if (typeof value === 'string') {
    return [readNonEmptyString(value, `${label}.allowed_tools`)];
  }
  if (Array.isArray(value) && value.every((item) => typeof item === 'string')) {
    return value.map((item) =>
      readNonEmptyString(item, `${label}.allowed_tools`)
    );
  }
  throw new Error(
...
export function readClaudeNativeToolRules(
  ...
  const targetAllow = readTargetNativeToolEscape(
    targetOptions,
    '_allow',
    label
  );
  ...
  const nativeAllow =
    targetAllow === false
      ? []
      : [
          ...readClaudeToolRuleList(
            readPortableToolEscape(record, '_allow', 'claude', label),
            label
          ),
  ...
  const nativeDeny =
    targetDeny === false
      ...
      : [
          ...readClaudeToolRuleList(
            readPortableToolEscape(record, '_deny', 'claude', label),
            label
          ),
  ...
  return {
    allow: [
      ...lowerPortableToolsForClaude(portable.allow, label),
      ...nativeAllow,
    ],
...
function hasTargetedValue(record: JsonRecord, key: string): boolean {
  ...
  return (
    isJsonRecord(value) &&
    TARGET_KEYS.some((target) => value[target] !== undefined)
  );
}
...
function readTargetedValue(
  ...
  if (!hasTargetedValue(record, key)) {
    return value;
  }
...
function readTargetNativeToolEscape(
  ...
  const tools = readToolsRecord(
    targetOptions,
    `${label} target tool_intent`,
    false
  );
  return tools?.[key];
}
...
function readToolsRecord(
  ...
  for (const key of Object.keys(value)) {
    ...
    if (allowPortablePolicy && (key === 'allow' || key === 'deny')) {
      continue;
    }
    const allowedKeys = allowPortablePolicy
      ...
      : '_allow and _deny';
    throw new Error(
      `skillset: expected ${label} to contain only ${allowedKeys} keys`
    );
  }
  return value;
}
...
function readClaudeToolRuleList(
  ...
  if (typeof value === 'string') {
    return [readNonEmptyString(value, `${label}.tool_intent`)];
  }
  if (Array.isArray(value)) {
    return value.map((item) => readClaudeToolRule(item, label));
  }
  if (isJsonRecord(value)) {
    return [readClaudeToolRule(value, label)];
  }
  throw new Error(
...
function readClaudeToolRule(value: JsonValue, label: string): string {
  ...
  if (isJsonRecord(value)) {
    ...
    if (typeof rule === 'string') {
      return readNonEmptyString(rule, `${label}.tool_intent.rule`);
    }
  }
  throw new Error(
    `skillset: expected ${label}.tool_intent _allow/_deny entries for Claude to be strings or objects with rule`
...
function readPortableToolsPolicy(
  ...
  const allow = readPortableToolsAction(
    tools.allow,
    `${label}.tool_intent.allow`
  );
...
function readPortableToolsAction(
  value: JsonValue | undefined,
  label: string
): JsonRecord | undefined {
  if (value === undefined || value === false) {
  ...
  if (!isJsonRecord(value)) {
    throw new Error(
      `skillset: expected ${label} to be an object of known portable tool keys`
    );
  }
  ...
  for (const [key, rawValue] of Object.entries(value)) {
    ...
    normalized[key] = normalizePortableToolValue(
      key,
      rawValue,
      `${label}.${key}`
    );
...
function normalizePortableToolValue(
  key: string,
  value: JsonValue,
  label: string
): JsonValue {
...
function normalizeBooleanStringList(
  value: JsonValue,
  label: string
): JsonValue {
  if (value === true) {
  ...
  throw new Error(
    `skillset: expected ${label} to be true, a string, or a string array`
  );
}
  ...
  return items.map((item) => {
    ...
    if (isJsonRecord(item)) {
      ...
      if (
        ...
        const parts = prefix.map((part) =>
          readNonEmptyString(part, `${label}.prefix`)
        );
...
function normalizeWebFetchToolValue(
  value: JsonValue,
  label: string
): JsonValue {
  if (value === true) {
  ...
  throw new Error(
    `skillset: expected ${label} to be true, a domain string/list, or domains object`
  );
}
...
function normalizeMcpToolValue(value: JsonValue, label: string): JsonValue {
  if (!isJsonRecord(value)) {
    throw new Error(
      `skillset: expected ${label} to be an object keyed by MCP server name`
    );
  }
  ...
  for (const [server, rawServerValue] of Object.entries(value)) {
    ...
    if (rawServerValue === undefined || rawServerValue === false) {
      continue;
    }
    ...
    if (typeof rawServerValue === 'string' || Array.isArray(rawServerValue)) {
      servers[serverName] = {
        tools: normalizeStringList(rawServerValue, `${label}.${serverName}`),
      };
      continue;
    }
    if (isJsonRecord(rawServerValue)) {
      servers[serverName] = {
        tools: normalizeStringList(
          rawServerValue.tools,
          `${label}.${serverName}.tools`
        ),
      };
      continue;
    }
...
function lowerPortableToolsForClaude(
  ...
  for (const [key, value] of Object.entries(tools)) {
    ...
    switch (key) {
      case 'edit': {
        rules.push(
          ...lowerPathRules('Edit', value, `${label}.tool_intent.${key}`)
        );
        break;
      }
      case 'read': {
        rules.push(
          ...lowerPathRules('Read', value, `${label}.tool_intent.${key}`)
        );
        break;
      }
      case 'search': {
...
function lowerShellRules(value: JsonValue, label: string): readonly string[] {
  ...
  if (!Array.isArray(value)) {
    throw new TypeError(
      `skillset: expected ${label} to be true or normalized shell entries`
    );
  }
  return value.map((entry) => {
    if (!isJsonRecord(entry) || typeof entry.pattern !== 'string') {
      throw new Error(
        `skillset: expected ${label} shell entries to contain pattern`
      );
    }
...
function lowerWebFetchRules(
  ...
  if (!isJsonRecord(value)) {
    throw new Error(
      `skillset: expected ${label} to be true or normalized web_fetch domains`
    );
  }
...
function lowerMcpRules(value: JsonValue, label: string): readonly string[] {
  if (!isJsonRecord(value)) {
    throw new Error(
      `skillset: expected ${label} to be normalized MCP server entries`
    );
  }
  const rules: string[] = [];
  for (const [server, rawServerValue] of Object.entries(value)) {
    if (!isJsonRecord(rawServerValue)) {
      throw new Error(
        `skillset: expected ${label}.${server} to be an object with tools`
      );
    }
...
function codexToolActionMetadata(
  ...
  if (portable !== undefined) {
    action.portable = portable;
  }
  if (native !== undefined) {
    action.target_native = native;
  }
...
function mergeCodexEscapeValues(
  ...
  if (targetSpecific === false) {
    return undefined;
  }
  if (targetSpecific === undefined) {
    return normalizedShared;
  }
  if (normalizedShared === undefined) {
