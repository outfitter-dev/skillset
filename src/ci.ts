...
import type { ChangeCheckIssue } from './change-entries';
 ...
 */
export async function ciSkillset(
  rootPath: string,
  options: CiOptions = {}
): Promise<CiReport> {
  ...
  const changeErrors = changeIssues.filter(
...
export function renderCiReportMarkdown(report: CiReport): string {
  ...
  if (report.ok && report.fixedPaths.length === 0) {
    lines.push(
      'All checks passed: source lint, change entries, and generated output are current.',
      ''
    );
    return `${lines.join('\n').trimEnd()}\n`;
  }
  ...
  if (report.fixedPaths.length > 0) {
    ...
    lines.push(
      '',
      'These files were regenerated from source the same way `skillset build --yes` would. Commit the rebuilt output if it is not committed for you.',
      ''
    );
  }
  ...
  if (hasDrift(report.drift)) {
    ...
    for (const path of report.drift.added) {
      lines.push(`- added: \`${path}\``);
    }
    for (const path of report.drift.changed) {
      lines.push(`- changed: \`${path}\``);
    }
    for (const path of report.drift.missing) {
      lines.push(`- missing: \`${path}\``);
    }
    for (const path of report.drift.removed) {
      lines.push(`- removed: \`${path}\``);
    }
    lines.push(
      '',
      'Run `skillset build --yes`, review the generated diff, and commit it.',
      ''
    );
  }
  ...
  if (report.lintIssues.length > 0) {
    ...
    lines.push(
      '',
      'Fix the source issues, then run `skillset lint` locally.',
      ''
    );
  }
