...
import { renderChangelogProjections } from './changelog';
import type { ChangelogProjection } from './changelog';
import {
  renderValidatedJson,
...
export async function renderBuildGraph(
  ...
  for (const plugin of graph.plugins) {
    rendered.push(
      ...(await renderPluginTarget(graph, plugin, 'claude', lockRoots))
    );
    rendered.push(
      ...(await renderPluginTarget(graph, plugin, 'codex', lockRoots))
    );
  }

  for (const skill of graph.standaloneSkills) {
    rendered.push(
      ...(await renderStandaloneSkill(graph, skill, 'claude', lockRoots))
    );
    rendered.push(
      ...(await renderStandaloneSkill(graph, skill, 'codex', lockRoots))
    );
  }
...
function coalesceRenderedFiles(
  ...
  for (const file of files) {
    ...
    if (bytesEqual(existing.content, file.content)) {
      continue;
    }
    throw new Error(
...
function renderRepositoryReadmes(graph: BuildGraph): readonly RenderedFile[] {
  const rendered: RenderedFile[] = [];
  if (
    graph.plugins.some((plugin) => shouldRenderPlugin(graph, plugin, 'claude'))
  ) {
    rendered.push(
      textFile(
  ...
  if (
    graph.plugins.some((plugin) => shouldRenderPlugin(graph, plugin, 'codex'))
  ) {
    rendered.push(
      textFile(
...
async function renderPluginTarget(
  ...
  if (!shouldRenderPlugin(graph, plugin, target)) {
    return [];
  }
  ...
  const enabledSkills = plugin.skills.filter(
    (skill) => skill.targets[target].enabled
  );
  ...
  rendered.push(
    ...(await renderPluginFeatureFiles(
      graph,
      plugin,
      target,
      basePath,
  ...
  rendered.push(
    ...(await copyPluginCompanionFiles(graph, plugin, target, basePath))
  );
  rendered.push(
    ...(await renderPluginIslands(
      graph,
      plugin,
      target,
      basePath,
...
function validateInternalPluginDependenciesForTarget(
  ...
  for (const dependency of pluginDependencies(graph, plugin)) {
    ...
    const dependencyPlugin = graph.plugins.find(
      (candidate) => candidate.id === dependency.name
    );
    if (dependencyPlugin === undefined) {
      continue;
    }
    if (shouldRenderPlugin(graph, dependencyPlugin, target)) {
      continue;
    }
    throw new Error(
...
function withOptionalSurfacePaths(
  ...
  if (enabledSkills.length > 0) {
    withPaths.skills = './skills/';
  }
  if (target === 'claude') {
    if (pluginHasPath(plugin, 'commands')) {
      withPaths.commands = './commands';
    }
    if (pluginHasPath(plugin, 'agents')) {
      withPaths.agents = './agents';
    }
    if (pluginHasPath(plugin, 'hooks/hooks.json')) {
      withPaths.hooks = './hooks/hooks.json';
    }
    if (pluginHasFeature(plugin, 'mcp')) {
      withPaths.mcpServers = './.mcp.json';
    }
    ...
    if (pluginHasPath(plugin, 'themes')) {
    ...
    if (pluginHasPath(plugin, '.app.json')) {
      withPaths.apps = './.app.json';
    }
  }
...
async function renderPluginSkillFiles(
  ...
  for (const file of await collectFiles(sourceDir)) {
    ...
    if (generatedCodexRelativeFiles.has(relativeFile)) {
      continue;
    }
    pushSkillRenderedFile(
      rendered,
...
async function renderClaudeProjectAgent(
  ...
  const skills =
    readStringArray(targetOptions, 'skills') ??
    readStringArray(agent.frontmatter, 'skills');
  const frontmatter = mergeRecords(
    mergeRecords(
      mergeRecords(
        stripAgentTargetOptions(
          stripSourceFrontmatter(agent.frontmatter, agent.sourcePath)
        ),
        {
          description:
            readString(targetOptions, 'description') ??
            readString(agent.frontmatter, 'description') ??
            agent.name,
  ...
  const targetPath = join(
    targetProjectRoot(graph, 'claude'),
    'agents',
    `${agent.outputName}.md`
  );
  return {
    file: textFile(
      ...
      renderValidatedMarkdown(
        frontmatter,
        body,
        `${relative(graph.rootPath, agent.sourcePath)} -> ${targetPath}`
      ),
      relative(graph.rootPath, agent.sourcePath)
    ),
    preprocessDependencies: projectAgentPreprocessDependencies(
      graph,
      preprocessDependencies
    ),
  };
...
async function renderCodexProjectAgent(
  ...
  if (initialPrompt?.includes('</initial_prompt>')) {
    throw new Error(
      `skillset: ${relative(graph.rootPath, agent.sourcePath)} initialPrompt must not contain </initial_prompt>`
    );
  }
  ...
  const targetPath = join(
    targetProjectRoot(graph, 'codex'),
    'agents',
    `${agent.outputName}.toml`
  );
...
async function renderCodexProjectAgentInstructions(
  ...
  const explicitInstructions = readString(
    targetOptions,
...
function projectAgentPreprocessDependencies(
  ...
  return [...dependencies]
    ...
    .map((path) => relative(graph.rootPath, path));
}

function renderCodexSkillsPreface(
  targetOptions: JsonRecord,
  skills: readonly string[]
): string {
  const bullets = skills.map((skill) => `- ${skill}`).join('\n');
  const template =
    readString(targetOptions, 'skillsPrefaceTemplate') ??
    'Load the following skills first, if available:\n\n{{skills}}';
  return template.includes('{{skills}}')
    ? template.replaceAll('{{skills}}', bullets)
    : `${template.trimEnd()}\n\n${bullets}`;
}
...
async function renderProjectIslands(
  ...
  const rendered: RenderedFile[] = [];
  for (const island of graph.projectIslands.filter(
    (item) => item.plugin === undefined
  )) {
    if (!graph.root.targets[island.target].enabled) {
      continue;
    }
...
async function renderPluginIslands(
  ...
  const rendered: RenderedFile[] = [];
  for (const island of graph.projectIslands.filter(
    (item) => item.plugin === plugin.id && item.target === target
  )) {
...
async function renderIslandFile(
  ...
  if (isTextIslandFile(island.relativePath)) {
    const preprocessDependencies = new Set<string>();
    const content = await renderTextIslandFile(
      graph,
      island,
      targetPath,
      preprocessDependencies
    ...
    return {
      file: textFile(
        targetPath,
        content,
        relative(graph.rootPath, island.sourcePath)
      ),
...
async function renderTextIslandFile(
  ...
  const source = await readFile(island.sourcePath, 'utf-8');
...
function rejectIslandTargetEscape(
  frontmatter: JsonRecord,
  island: SourceIslandFile
): void {
  if (
    frontmatter.claude !== undefined ||
    frontmatter.codex !== undefined ||
    frontmatter.targets !== undefined
  ) {
    throw new Error(
...
async function renderStandaloneSkill(
  ...
  if (!shouldRenderStandaloneSkill(graph, skill, target)) {
    return [];
  }
...
async function renderSkillResources(
  ...
  for (const resource of skill.resources) {
    for (const file of await copyPath(
      resource.sourcePath,
      join(targetSkillDir, resource.targetPath)
    )) {
...
function pushSkillRenderedFile(
  ...
  const relativeFile = normalizeRenderedRelativePath(
    relative(targetSkillDir, file.path)
  );
...
async function renderSkillMarkdown(
  ...
  const base = mergeRecords(
    stripSourceFrontmatter(skill.frontmatter, skill.sourcePath),
    {
      description:
  ...
  const withReferences =
    references === undefined ? base : mergeRecords(base, { references });
  const withClaudePolicy =
    target === 'claude'
      ? mergeRecords(
          withReferences,
          renderClaudeSkillPolicy(skill, targetOptions)
        )
      : withReferences;
  const withPortable = graph.root.compile.skillset.metadata
    ? mergeRecords(withClaudePolicy, {
        metadata: { generated: GENERATED_BY, version },
      })
    : withClaudePolicy;
...
function renderClaudeSkillPolicy(
  skill: SourceSkill,
  targetOptions: JsonRecord
): JsonRecord {
  ...
  const nativeTools = readClaudeNativeToolRules(
    skill.frontmatter,
    targetOptions,
    label
  );
  ...
  const allow = [
    ...(allowedTools !== undefined && allowedTools !== false
      ? allowedTools
      : []),
    ...nativeTools.allow,
  ];
...
async function renderCodexSkillAgentFile(
  ...
  const source = hasSourceOpenAi
    ? parseYamlRecord(
        await preprocessText(await readFile(sourceOpenAiPath, 'utf-8'), {
          frontmatter: skill.frontmatter,
          rootPath: graph.rootPath,
  ...
  return textFile(
    ...
    renderValidatedYaml(
      merged,
      `${relative(graph.rootPath, sourceOpenAiPath)} -> ${join(targetSkillDir, 'agents/openai.yaml')}`
    ),
...
function renderCodexSkillToolsFile(
  ...
  const label = relative(graph.rootPath, skill.sourcePath);
  const tools = readCodexToolMetadata(
    skill.frontmatter,
...
async function renderCodexAgentsFiles(
  ...
  const rendered: RenderedFile[] = [];
  for (const [destination, rules] of [...destinations.entries()].toSorted(
...
async function renderClaudeRuleMarkdown(
  ...
  return stringifyOptionalMarkdown(
    frontmatter,
    await renderRuleBody(graph, rule, outputPath)
  );
}
...
async function renderCodexAgentsMarkdown(
  ...
  const sections = rules.map(async (rule) => ({
    body: await renderRuleBody(graph, rule, outputPath),
    rule,
  }));
...
async function codexBaseForPattern(
  ...
  if (!hasGlobSyntax(normalized)) {
    return dirnameOrRoot(normalized);
  }
...
async function matchingRepoFiles(
  ...
  for await (const match of glob.scan({
    ...
    if (isIgnoredRuleMatch(graph, normalized)) {
      continue;
    }
...
async function renderPluginFeatureFiles(
  ...
  for (const feature of plugin.features) {
    if (!pluginFeatureSupportsTarget(feature, target)) {
      continue;
    }
    const files = (
      await copyPath(feature.sourcePath, join(basePath, feature.targetPath))
    )
      ...
      .map((file) =>
        feature.key === 'mcp'
          ? {
              ...file,
              sourcePath: relative(graph.rootPath, feature.sourcePath),
            }
          : file
      );
    rendered.push(...files);
...
function pluginFeatureSupportsTarget(
...
async function renderCodexHookFile(
  ...
  if (!(await exists(canonicalSource))) {
    return undefined;
  }

  await validateHookJson(graph, canonicalSource, 'codex');
  const parsed = JSON.parse(
    await readFile(canonicalSource, 'utf-8')
  ) as JsonValue;
  const normalized =
...
async function validateHookJson(
  ...
  try {
    ...
    throw new Error(
      `skillset: ${targetLabel} hook file ${label} is not valid JSON: ${message}`,
      { cause: error }
    );
  }
...
async function copyPath(
  ...
  if (stats.isFile()) {
    return [{ content: await readFile(sourcePath), path: targetPath }];
  }
...
function renderLockFiles(
  ...
  for (const [outputRoot, lock] of [...lockRoots.entries()].toSorted(
    ([left], [right]) => compareStrings(left, right)
  )) {
    const value: JsonRecord = {
      ...
      items: lock.items
        ...
        .toSorted((left, right) =>
          compareStrings(String(left.outputPath), String(right.outputPath))
        ),
      outputRoot,
      schemaVersion: 1,
    ...
    rendered.push(
      textFile(
        ...
        renderValidatedJson(value, `${outputRoot}/.skillset.lock`)
      )
    );
  }
...
function lockItemForPlugin(args: {
  ...
  const dependencyHashSummaries = pluginDependencyHashSummaries(
    args.graph,
    args.plugin,
    args.target
  );
...
async function lockItemForPluginFeature(args: {
  ...
  return {
    ...
    outputPath: relative(
      args.outputRoot,
      join(args.outputRoot, 'plugins', args.plugin.id, args.feature.targetPath)
    ),
    ...
    ...(args.feature.sourcePointer === undefined
      ? {}
      : { sourcePointer: args.feature.sourcePointer }),
    targetState:
...
async function lockItemForSkill(args: {
  ...
  return {
    ...
    outputHash: hashRenderedFiles(args.outputRoot, args.files),
    outputPath:
      files.find((file) => file.endsWith('/SKILL.md')) ?? files[0] ?? '',
...
function stripUndefinedLockItem(item: LockItem): JsonRecord {
  const value: Record<string, JsonValue | undefined> = {
    dependencies:
      item.dependencies === undefined ? undefined : [...item.dependencies],
    feature: item.feature,
    files: [...item.files],
    includedSkills:
      item.includedSkills === undefined ? undefined : [...item.includedSkills],
    kind: item.kind,
    name: item.name,
    origin: item.origin,
    ...
    preprocessDependencies:
      item.preprocessDependencies === undefined
        ? undefined
        : [...item.preprocessDependencies],
    skippedSkills:
      item.skippedSkills === undefined ? undefined : [...item.skippedSkills],
...
async function hashPluginFeatureSource(
  feature: SourcePluginFeature
): Promise<string> {
  const hash = createHash('sha256');
...
function hashRules(rules: readonly SourceRule[]): string {
  ...
  for (const rule of [...rules].toSorted((left, right) =>
...
function hasRenderableContent(path: string): boolean {
  ...
  for (const entry of readdirSync(path)) {
    if (hasRenderableContent(join(path, entry))) {
      return true;
    }
  }
...
function textFile(
  ...
  return sourcePath === undefined
    ? { content: textEncoder.encode(content), path }
    : { content: textEncoder.encode(content), path, sourcePath };
}
