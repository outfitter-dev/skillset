...
export async function initSkillset(
...
export async function createSkillset(
  ...
  if (await pathExists(rootPath)) {
    ...
    if (!stats.isDirectory()) {
      throw new Error(
        `skillset: create target exists and is not a directory: ${rootPath}`
      );
    }
...
export function defaultGlobalSourcePath(
...
async function applySetupPlan(
  ...
  for (const file of plannedFiles) {
    ...
    if (existing !== undefined && existing !== file.content) {
      ...
      throw new Error(
        `skillset: refusing to overwrite existing setup file ${file.path}`
      );
    }
    files.push({
      path: file.path,
      status: existing === undefined ? 'create' : 'exists',
    });
  }
  ...
  const importCandidates =
    kind === 'init' ? await detectImportCandidates(rootPath) : [];
...
async function initRootPath(options: SetupOptions): Promise<string> {
  ...
  if (options.rootPath !== undefined) {
    return resolve(cwd, options.rootPath);
  }
  if (options.useGitRoot === false) {
    return cwd;
  }
...
async function validateExistingRootConfig(path: string): Promise<void> {
  ...
  validateConfigDocument(parseYamlRecord(content, path), path, {
    allowCompile: true,
  });
}
...
async function detectImportCandidates(
  ...
  if (
    (await pathExists(join(rootPath, '.claude-plugin/plugin.json'))) ||
    (await pathExists(join(rootPath, '.codex-plugin/plugin.json')))
  ) {
    candidates.push({ kind: 'plugin', path: '.' });
  }
  return candidates.toSorted((left, right) => compareCandidate(left, right));
}
...
async function isManagedCandidate(path: string): Promise<boolean> {
  return (
    (await pathExists(join(path, '.skillset.lock'))) ||
    (await pathExists(join(dirname(path), '.skillset.lock')))
  );
}

function compareCandidate(
  left: SetupImportCandidate,
  right: SetupImportCandidate
): number {
  return `${left.kind}:${left.path}` < `${right.kind}:${right.path}`
    ? -1
    : `${left.kind}:${left.path}` > `${right.kind}:${right.path}`
      ? 1
      : 0;
}
...
function setupFiles(
  ...
  if (include.includes('ci')) {
    files.push({ content: renderCiWorkflow(), path: CI_WORKFLOW_PATH });
  }
...
function createRootPath(options: SetupOptions): string {
  if (options.rootPath !== undefined) {
    return resolve(options.cwd ?? process.cwd(), options.rootPath);
  }
  if (options.global === true) {
    return defaultGlobalSourcePath(options.homeDir);
  }
...
function defaultSetupName(kind: SetupReport['kind'], rootPath: string): string {
  if (
    kind === 'create' &&
    basename(rootPath) === 'src' &&
    basename(join(rootPath, '..')) === '.skillset'
  ) {
...
function normalizeTargets(
  targets: readonly TargetName[] | undefined
): readonly TargetName[] {
  ...
  for (const target of targets) {
    if (target !== 'claude' && target !== 'codex') {
      throw new Error(
        `skillset: unsupported setup target ${JSON.stringify(target)}`
      );
    }
    seen.add(target);
  }
...
async function readExistingFile(path: string): Promise<string | undefined> {
  try {
    ...
    if (!stats.isFile()) {
      throw new Error(
        `skillset: refusing to overwrite non-file setup path ${path}`
      );
    }
