...
const PRE_PUSH_COMMAND =
  'skillset change check --since origin/main && skillset check && skillset doctor';
...
const RUNTIME_SOURCE_STATUS_COMMAND = `git status --porcelain=v1 --untracked-files=all -- ${RUNTIME_SOURCE_PATHS.join(' ')}`;
const RUNTIME_POST_TOOL_USE_COMMAND = `${RUNTIME_SOURCE_STATUS_COMMAND} | grep -q . && skillset change status --root . || true`;
const RUNTIME_STOP_COMMAND = `if ${RUNTIME_SOURCE_STATUS_COMMAND} | grep -q .; then skillset change check --root . && skillset check --root .; fi`;

export function renderHookPrint(options: HookPrintOptions): string {
  validateHookPrintOptions(options);
  if (options.agentRuntime) {
    return renderAgentRuntimeSnippet(options.target);
  }
  if (options.runner === undefined) {
    throw new Error(
      'skillset: hooks print requires --runner or --agent-runtime'
    );
  }
  const preCommit =
    options.preCommit || (!options.preCommit && !options.prePush);
...
function validateHookPrintOptions(options: HookPrintOptions): void {
  if (options.agentRuntime && options.runner !== undefined) {
    throw new Error(
      'skillset: hooks print --agent-runtime cannot be combined with --runner'
    );
  }
  if (options.agentRuntime) {
    if (options.target === undefined) {
      throw new Error(
        'skillset: hooks print --agent-runtime requires --target claude or --target codex'
      );
    }
    if (options.preCommit || options.prePush) {
      throw new Error(
        'skillset: hooks print --agent-runtime cannot be combined with --pre-commit or --pre-push'
      );
    }
    return;
  }
  if (options.target !== undefined) {
    throw new Error(
      'skillset: hooks print --target is only supported with --agent-runtime'
    );
  }
}
...
function renderRunnerSnippet(
  ...
  if (runner === 'lefthook') {
    return renderLefthookSnippet(options);
  }
  if (runner === 'husky') {
    return renderHuskySnippet(options);
  }
  if (runner === 'pre-commit') {
    return renderPreCommitSnippet(options);
  }
  return renderGitSnippet(options);
}

function renderLefthookSnippet(options: {
  readonly preCommit: boolean;
  readonly prePush: boolean;
}): string {
...
function renderHuskySnippet(options: {
  readonly preCommit: boolean;
  readonly prePush: boolean;
}): string {
...
function renderPreCommitSnippet(options: {
  readonly preCommit: boolean;
  readonly prePush: boolean;
}): string {
  const lines = [
...
function renderGitSnippet(options: {
  readonly preCommit: boolean;
  readonly prePush: boolean;
}): string {
  const sections: string[] = [];
  ...
  if (options.prePush) {
    sections.push(
      ['# .git/hooks/pre-push', '#!/bin/sh', 'set -eu', PRE_PUSH_COMMAND].join(
        '\n'
      )
    );
  }
...
function renderAgentRuntimeSnippet(target: TargetName | undefined): string {
  if (target === undefined) {
    throw new Error('skillset: hooks print --agent-runtime requires --target');
  }
  const note =
