...
test('ci reports generated drift without writing when fix is off', async () => {
  ...
  const edited = `${await readFile(generatedPath, 'utf-8')}\nhand edit\n`;
  ...
  expect(report.fixedPaths).toEqual([]);
  expect(await readFile(generatedPath, 'utf-8')).toBe(edited);
...
test('ci --fix rebuilds drifted generated output mechanically', async () => {
  const root = await builtFixture();
  const generatedPath = join(root, GENERATED_SKILL);
  const original = await readFile(generatedPath, 'utf-8');
  ...
  expect(await readFile(generatedPath, 'utf-8')).toBe(original);
  const markdown = renderCiReportMarkdown(report);
...
test('ci --fix leaves drift untouched when change entries are missing', async () => {
  ...
  expect(report.changeIssues.some((issue) => issue.severity === 'error')).toBe(
    true
  );
  expect(await readFile(join(root, GENERATED_SKILL), 'utf-8')).not.toContain(
    'Edited body.'
  );
  const markdown = renderCiReportMarkdown(report);
  expect(markdown).toContain('### Stale generated output');
...
test('ci surfaces an unresolvable change baseline as a change error', async () => {
  ...
  expect(renderCiReportMarkdown(report)).toContain(
    '### Change check could not run'
  );
});
...
test('ci CLI exits nonzero on drift and writes the markdown report', async () => {
  ...
  const failed = await runSkillsetCli(
    'ci',
    '--root',
    root,
    '--since',
  ...
  expect(await readFile(reportPath, 'utf-8')).toStartWith(CI_REPORT_MARKER);

  const fixed = await runSkillsetCli(
    'ci',
    '--fix',
    '--root',
    root,
  ...
  expect(await readFile(reportPath, 'utf-8')).toContain('rebuilt mechanically');
...
test('ci CLI rejects misplaced and unsupported flags', async () => {
  ...
  expect(reportOutsideCi.stderr).toContain(
    '--report is only supported with ci'
  );
  ...
  expect(sinceWithBuild.stderr).toContain(
    '--since is only supported with ci or change commands'
  );
});
...
test('init --include ci scaffolds a valid workflow and keeps user edits', async () => {
  ...
  const rerun = await initSkillset({
    cwd: root,
    include: ['ci'],
    useGitRoot: false,
    write: true,
  ...
  expect(await readFile(workflowPath, 'utf-8')).toBe(customized);
});
...
test('init without --include ci does not plan a workflow', async () => {
  ...
  expect(report.files.some((file) => file.path === CI_WORKFLOW_PATH)).toBe(
    false
  );
});
