...
`,
  ...
  await expect(loadBuildGraph(root)).rejects.toThrow(
    'skillset.schema is the source schema marker'
  );
});
...
`,
  ...
  await expect(loadBuildGraph(root)).rejects.toThrow(
    'uses unsupported skillset.name; use top-level name'
  );
});
...
`,
  ...
  await expect(loadBuildGraph(root)).rejects.toThrow(
    'uses unsupported skillset.version; use top-level version'
  );
});
...
`,
  ...
  await buildSkillset(root);
  expect(
    await readFile(join(root, '.claude/rules/global.md'), 'utf-8')
  ).toContain('Be tidy.');
  expect(await readFile(join(root, 'AGENTS.md'), 'utf-8')).toContain(
    'Be tidy.'
  );
});
...
`,
  ...
  await expect(loadBuildGraph(root)).rejects.toThrow(
    '.skillset/rules is not supported'
  );
});
...
`,
  ...
  await buildSkillset(root);
  const skill = await readFile(
    join(root, '.claude/skills/intent/SKILL.md'),
    'utf-8'
  );
...
`,
  ...
  await expect(buildSkillset(root)).rejects.toThrow(
    'uses unsupported tools; use tool_intent'
  );
});
...
`,
  ...
  await expect(buildSkillset(root)).rejects.toThrow(
    'unknown portable tool key teleport'
  );
});
...
`,
  ...
  const claudeHook = await readFile(
    join(root, 'plugins-claude/plugins/alpha/hooks/hooks.json'),
    'utf-8'
  );
...
`,
    ...
    await expect(loadBuildGraph(root)).rejects.toThrow(
      'uses unsupported root hooks.json'
    );
  }
});
...
test('SET-14: Codex plugin manifest interface uses documented camelCase fields', async () => {
  ...
  const manifest = JSON.parse(
    await readFile(
      join(root, 'plugins-codex/plugins/widget/.codex-plugin/plugin.json'),
      'utf-8'
    )
...
`,
  ...
  const manifest = JSON.parse(
    await readFile(
      join(root, 'plugins-codex/plugins/plain/.codex-plugin/plugin.json'),
      'utf-8'
    )
...
test('SET-14: Claude plugin manifest emits the documented top-level fields', async () => {
  ...
  const manifest = JSON.parse(
    await readFile(
      join(root, 'plugins-claude/plugins/widget/.claude-plugin/plugin.json'),
      'utf-8'
    )
...
test('SET-10: skill import reports copied files and classifies frontmatter', async () => {
  ...
  expect(report.preservedTargetNativeFields).toContain(
  ...
  const copied = await readFile(join(report.targetPath, 'SKILL.md'), 'utf-8');
...
test('SET-10: importing a SKILL.md path copies the full skill directory', async () => {
  ...
  await Bun.write(
    join(external, 'full-skill/references/notes.md'),
    'Imported reference.\n'
  );
  await Bun.write(
    join(external, 'full-skill/scripts/run.sh'),
    '#!/usr/bin/env bash\n'
  );
  ...
  expect(
    await Bun.file(
      join(root, '.skillset/skills/full-skill/references/notes.md')
    ).exists()
  ).toBe(true);
});
...
test('SET-10: inferred skills-root import copies each skill and dedupes symlinked directories', async () => {
  ...
  const external = await mkdtemp(join(tmpdir(), 'skillset-import-src-'));
  await Bun.write(
    join(external, 'skills/other/SKILL.md'),
    '---\nname: other\ndescription: Other.\n---\n\nOther.\n'
  );
  await Bun.write(
  ...
  await symlink(
    join(external, 'skills/shared'),
    join(external, 'skills/shared-alias'),
    'dir'
  );
  ...
  expect(report.imports.map((entry) => entry.name).toSorted()).toEqual([
    'other',
    'shared',
  ]);
  expect(
    await Bun.file(join(root, '.skillset/skills/other/SKILL.md')).exists()
  ).toBe(true);
  expect(
    await Bun.file(join(root, '.skillset/skills/shared/SKILL.md')).exists()
  ).toBe(true);
});
...
test('SET-10: plugin import reports the config and copied files', async () => {
  ...
  expect(report.inferredSourceFields).toEqual(
    expect.arrayContaining(['claude', 'codex', 'skillset'])
  );
...
test('SET-10: inferred plugin-root import writes source config for native plugin manifests', async () => {
  ...
  await Bun.write(
    ...
    JSON.stringify({
      description: 'Native widget plugin.',
      name: 'Widget',
      version: '0.8.0',
    })
  );
  ...
  const config = await readFile(
    join(root, '.skillset/plugins/widget/skillset.yaml'),
    'utf-8'
  );
  ...
  expect(
    await Bun.file(
      join(root, '.skillset/plugins/widget/.claude-plugin/plugin.json')
    ).exists()
  ).toBe(true);
...
test('SET-10: failed imports do not leave source target directories', async () => {
  ...
  await expect(
    importSource({
      kind: 'skill',
      rootPath: root,
      sourcePath: join(external, 'not-a-skill.md'),
    })
  ).rejects.toThrow('importing a file is only supported');

  expect(
    await Bun.file(join(root, '.skillset/skills/partial-import')).exists()
  ).toBe(false);
...
`,
  ...
  await buildSkillset(root);
  expect(await diffSkillset(root)).toEqual({
    added: [],
    changed: [],
    missing: [],
    removed: [],
...
`,
  ...
  const dryRun = await runSkillsetCli(
    'build',
    '--root',
    root,
    '--yes',
...
test('SET-25: CLI help succeeds before command validation', async () => {
  ...
  expect(rootHelp.stdout).not.toContain(
    'skillset change status [--since <ref>] [--scope <scope>]'
  );
  expect(rootHelp.stdout).not.toContain(
    'skillset change check [@ref|--ref <ref>] [--since <ref>] [--scope <scope>]'
  );
  expect(rootHelp.stdout).toContain('skillset explain <path>');
  expect(rootHelp.stdout).toContain(
    'skillset import <path> [--kind <skill|skills|plugin|plugins>]'
  );
...
test('SET-41: hooks print emits additive runner snippets', async () => {
  for (const [runner, marker] of [
    ...
    const printed = await runSkillsetCli(
      'hooks',
      'print',
      '--runner',
      runner,
    ...
    if (runner === 'pre-commit') {
      expect(printed.stdout).toContain('entry: sh -c');
    }
  }
});

test('SET-41: hooks print emits target runtime suggestions without installing', async () => {
  const claude = await runSkillsetCli(
    'hooks',
    'print',
    '--target',
    'claude',
  ...
  const codex = await runSkillsetCli(
    'hooks',
    'print',
    '--target',
    'codex',
  ...
  const scoped = await runSkillsetCli(
    'hooks',
    'print',
    '--runner',
    'git',
  ...
  const since = await runSkillsetCli(
    'hooks',
    'print',
    '--runner',
    'git',
  ...
  const named = await runSkillsetCli(
    'hooks',
    'print',
    '--runner',
    'git',
  ...
  const importKind = await runSkillsetCli(
    'hooks',
    'print',
    '--runner',
    'git',
...
`,
  ...
  const firstLatest = JSON.parse(
    await readFile(join(root, '.skillset/build/tests/latest.json'), 'utf-8')
  ) as {
    runId: string;
    runPath: string;
  ...
  expect(await fileExists(join(root, firstLatest.runPath, 'report.json'))).toBe(
    true
  );
  expect(
    await fileExists(join(root, '.skillset/build/tests/latest/report.json'))
  ).toBe(true);
  expect(
    await fileExists(
      join(
        root,
        '.skillset/build/tests/latest/workspace/.claude/skills/demo/SKILL.md'
      )
    )
  ).toBe(true);
  expect(
    await fileExists(
      join(
        root,
        '.skillset/build/tests/latest/workspace/.agents/skills/demo/SKILL.md'
      )
    )
  ).toBe(false);
  expect(await fileExists(join(root, '.claude/skills/demo/SKILL.md'))).toBe(
    false
  );
  const firstReport = JSON.parse(
    await readFile(join(root, firstLatest.runPath, 'report.json'), 'utf-8')
  ) as {
  ...
  expect(await fileExists(join(root, secondLatest.runPath, 'report.md'))).toBe(
    true
  );
});
...
`,
  ...
  const report = JSON.parse(
    await readFile(
      join(root, '.skillset/build/tests/latest/report.json'),
      'utf-8'
    )
  ) as {
    ok: boolean;
...
test('SET-50: skillset test rejects build scope and write flags', async () => {
  ...
  expect(scoped.stderr).toContain(
    'build/write options are not supported with test'
  );
...
`
  ...
  await writeFile(
    join(root, '.skillset/skills/unstaged/SKILL.md'),
    `
---
name: unstaged
...
`
  ...
  const status = await runSkillsetCli(
    'change',
    'status',
    '--staged',
    '--root',
  ...
  const demoHash = stagedStatus.sourceChanges.find(
    (change) => change.id === 'skill:demo'
  )?.currentHash;
...
`,
  ...
  expect(
    first.sourceUnits.map((unit) => ({
      hash: unit.hash,
      id: unit.id,
      kind: unit.kind,
    }))
  ).toEqual(
    second.sourceUnits.map((unit) => ({
      hash: unit.hash,
      id: unit.id,
      kind: unit.kind,
    }))
...
`
  ...
  expect(report.sourceChanges.map((change) => change.id)).toContain(
    'skill:demo'
  );
...
`,
  ...
  expect(checked.stderr).toContain(
    'repo:packages/docs-cli/package.json is 3.1.0'
  );
});
...
`,
  ...
  await expect(loadBuildGraph(root)).rejects.toThrow(
    'OR ranges are not supported in v1'
  );
});
...
`
  ...
  const demo = status.sourceChanges.find(
    (change) => change.id === 'skill:demo'
  );
  expect(demo?.currentHash).toBeDefined();
  expect(demo?.currentRegions).toContainEqual({
    name: 'supports',
...
`,
  ...
  const claudeManifest = await readFile(
    join(root, 'plugins-claude/plugins/audit/.claude-plugin/plugin.json'),
    'utf-8'
  );
  ...
  const codexSkill = await readFile(
    join(root, 'plugins-codex/plugins/audit/skills/audit-skill/SKILL.md'),
    'utf-8'
  );
  ...
  expect(codexSkill).toContain(
    'external-tools range ^2.1.0 marketplace acme external'
  );
  ...
  expect(listed.stdout).toContain(
    'deps:external-tools range ^2.1.0 marketplace acme external'
  );
  ...
  expect(explained.stdout).toContain(
    'dependencies: external-tools range ^2.1.0 marketplace acme external'
  );
  expect(explained.stdout).toContain('secrets-vault range =1.2.3 internal');

  const auditLockSourceHash = async (): Promise<string> => {
    const lock = JSON.parse(
      await readFile(join(root, 'plugins-claude/.skillset.lock'), 'utf-8')
    ) as {
      items: { outputPath?: string; sourceHash?: string }[];
    };
    return (
      lock.items.find(
        (item) => item.outputPath === 'plugins/audit/.claude-plugin/plugin.json'
      )?.sourceHash ?? ''
    );
  };
  const originalHash = await auditLockSourceHash();
  await writeFile(
    join(root, '.skillset/plugins/secrets-vault/skillset.yaml'),
    `
skillset:
  name: secrets-vault
  version: 1.2.3
...
`,
  ...
  await expect(loadBuildGraph(root)).rejects.toThrow(
    'requires range or unversioned: true'
  );
});
...
`,
  ...
  await expect(buildSkillset(root)).rejects.toThrow(
    'would overwrite generated dependency metadata'
  );
});
...
`,
  ...
  await expect(buildSkillset(root)).rejects.toThrow(
    'has no enabled Codex skills to carry the dependency notice'
  );
});
...
`,
  ...
  await Bun.write(
    join(root, '.skillset/shared/common.md'),
    'Changed partial.\n'
  );
  ...
  const instruction = report.sourceUnits.find(
    (unit) => unit.id === 'instruction:root'
  );
  ...
  expect(report.generatedDrift.changed).toContain(
...
`,
  ...
  await Bun.write(
    join(root, '.skillset/skills/two/SKILL.md'),
    '---\nname: two\ndescription: Two.\n---\n\nTwo changed.\n'
  );
  const report = await changeStatus(root, { since: 'HEAD' });
...
`,
  ...
  const report = await changeStatus(root, { since: 'HEAD' });
  const demo = report.sourceChanges.find(
    (change) => change.id === 'skill:demo'
  );
  expect(demo?.currentHash).toBeDefined();
...
`,
  ...
  expect(demo?.currentHash).toBeDefined();
  expect(demo?.baselineRegions).toContainEqual({
    name: 'dependencies',
...
`
  ...
  const tooShort = await runSkillsetCli(
    'change',
    'check',
    '@abcde',
    '--root',
...
`,
  ...
  await writeFile(
    reasonPath,
    'Clarified the demo skill behavior and documented why this source edit needs a patch entry.\n',
    'utf-8'
  );
  ...
  const list = await runSkillsetCli(
    'change',
    'list',
    '--root',
    root,
...
`,
  ...
  const pending = await readFile(
    join(root, '.skillset/changes/pending', files[0] ?? ''),
    'utf-8'
  );
...
`
  ...
  await writeFile(
    ...
    `${[
      ...
      JSON.stringify({
        ...
        id: '123456abcdef',
        reason:
          'Applied history remains inspectable through the history command.',
        scope: 'skill:demo',
      }),
    ].join('\n')}\n`,
    'utf-8'
  ...
  const show = await runSkillsetCli(
    'change',
    'show',
    '@abcdef',
    '--root',
  ...
  const history = await runSkillsetCli(
    'change',
    'history',
    '@123456',
    '--root',
  ...
  const pendingHistory = await runSkillsetCli(
    'change',
    'history',
    '@abcdef',
    '--root',
  ...
  expect(pendingHistory.stderr).toContain(
    'is pending; no applied history entry'
  );
});
...
`
  ...
  await writeHistory(root, [
    ...
    },
    {
      ...
      id: '222222bbbbbb',
      reason:
        'Recorded an audit-only correction that should still appear in history.',
      scope: 'skill:demo',
    },
  ]);
  ...
  const changelog = await readFile(
    join(root, '.skillset/skills/demo/CHANGELOG.md'),
    'utf-8'
  );
  ...
  expect(changelog.indexOf('222222bbbbbb')).toBeLessThan(
    changelog.indexOf('111111aaaaaa')
  );
  ...
  await writeHistory(root, [
    {
      bump: 'minor',
      evidence: [
        { scope: 'plugin.alpha.skill:demo', sourceHash: 'sha256:child' },
      ],
      id: '333333cccccc',
      reason:
        'Updated the plugin child skill and projected it into the plugin changelog.',
      scope: 'plugin.alpha.skill:demo',
    },
  ]);

  await buildSkillset(root);
  const pluginChangelog = await readFile(
    join(root, '.skillset/plugins/alpha/CHANGELOG.md'),
    'utf-8'
  );
...
`
  ...
  expect(checked.stdout).toContain(
    'scope standalone-skill:demo does not match a known source unit'
  );
...
`,
  ...
  await writeHistory(root, [
    ...
    },
    {
      bump: 'minor',
      evidence: [
        { scope: 'plugin.alpha.skill:child', sourceHash: 'sha256:child' },
      ],
      id: '555555eeeeee',
  ...
  for (const id of ['skill:demo', 'plugin.alpha.skill:child', 'plugin:alpha']) {
    expect(sourceInventoryUnit(after, id)).toEqual(
      sourceInventoryUnit(before, id)
    );
  }
});
...
`
  ...
  expect(
    await Bun.file(join(root, '.skillset/changes/state.json')).exists()
  ).toBe(false);
  ...
  const applied = await runSkillsetCli(
    'release',
    'apply',
    '--yes',
    '--root',
  ...
  expect(
    await Bun.file(join(root, '.skillset/changes/pending/demo.md')).exists()
  ).toBe(false);

  const state = JSON.parse(
    await readFile(join(root, '.skillset/changes/state.json'), 'utf-8')
  ) as {
  ...
  const releases = await readFile(
    join(root, '.skillset/changes/releases.jsonl'),
    'utf-8'
  );
  ...
  const second = await runSkillsetCli(
    'release',
    'apply',
    '--yes',
    '--root',
  ...
  expect(
    await readFile(join(root, '.skillset/changes/history.jsonl'), 'utf-8')
  ).toBe(history);

  const releasedStatus = await changeStatus(root);
  expect(releasedStatus.sourceChanges.map((change) => change.id)).not.toContain(
    'skill:demo'
  );
  ...
  expect(unreleasedStatus.sourceChanges.map((change) => change.id)).toContain(
    'skill:demo'
  );
});
...
`,
  ...
  const status = await changeStatus(root, { since: 'HEAD' });
  const child = status.sourceChanges.find(
    (change) => change.id === 'plugin.alpha.skill:child'
  );
...
`
  ...
  expect(plan.stdout).toContain(
    'skill(plugin:alpha): child: 0.1.0 -> 0.2.0 (minor)'
  );
  ...
  expect(
    await readFile(join(root, '.skillset/plugins/alpha/CHANGELOG.md'), 'utf-8')
  ).toContain('## dddd11112222');
  expect(
    await readFile(
  ...
  expect(
    await readFile(
      join(root, 'plugins-claude/plugins/alpha/skills/child/SKILL.md'),
      'utf-8'
    )
  ).toContain('version: 0.2.0');
});
...
`,
  ...
  await Bun.write(
    join(root, '.skillset/skills/audit/SKILL.md'),
    '---\nname: audit\ndescription: Audit.\nversion: 0.1.0\n---\n\nAudit-only change.\n'
  );
  await Bun.write(
    join(root, '.skillset/skills/ignored/SKILL.md'),
    '---\nname: ignored\ndescription: Ignored.\nversion: 0.1.0\n---\n\nIgnored change.\n'
  );
  const status = await changeStatus(root, { since: 'HEAD' });
  const audit = status.sourceChanges.find(
    (change) => change.id === 'skill:audit'
  );
  const ignored = status.sourceChanges.find(
    (change) => change.id === 'skill:ignored'
  );
...
`
  ...
  expect(
    await readFile(join(root, '.skillset/skills/audit/CHANGELOG.md'), 'utf-8')
  ).toContain('## 333333ffffff');
  expect(
    await Bun.file(join(root, '.skillset/skills/ignored/CHANGELOG.md')).exists()
  ).toBe(false);
...
`,
    ...
    JSON.stringify(
      {
        ...
        scopes: {
          'skill:deleted': {
            sourceHash: sourceInventoryUnit(initialInventory, 'skill:deleted')
              .hash,
            version: '1.2.3',
          },
        },
  ...
  const status = await changeStatus(root, { since: 'HEAD' });
  const deleted = status.sourceChanges.find(
    (change) => change.id === 'skill:deleted'
  );
...
`
  ...
  const restoredSkill = await readFile(
    join(root, '.claude/skills/deleted/SKILL.md'),
    'utf-8'
  );
...
`,
  ...
  expect(scoped.exitCode).toBe(1);
  expect(scoped.stderr).toContain(
    '--scope is not supported with release commands yet'
  );
});
  ...
  await writeHistory(root, [
    {
      bump: 'patch',
      evidence: [
        { scope: 'plugin.alpha.feature:mcp', sourceHash: 'sha256:feature' },
      ],
      id: '666666ffffff',
      reason:
        'Released the plugin MCP server definition so setup requirements appear in the plugin changelog.',
...
`,
  ...
  await expect(checkSkillset(root)).rejects.toThrow(
    'missing managed generated file: .claude/skills/demo/SKILL.md'
  );
});
...
`,
  ...
  const scopedWrite = await runSkillsetCli(
    'build',
    '--root',
    root,
    '--scope',
  ...
  const conflicting = await runSkillsetCli(
    'build',
    '--root',
    root,
    '--updated',
  ...
  const unknownScope = await runSkillsetCli(
    'build',
    '--root',
    root,
    '--scope',
...
`,
  ...
  const pluginsOnly = await runSkillsetCli(
    'build',
    '--root',
    root,
    '--scope',
  ...
  expect(
    await Bun.file(
      join(root, 'plugins-claude/plugins/alpha/skills/plugin-skill/SKILL.md')
    ).exists()
  ).toBe(true);
  expect(
    await Bun.file(join(root, '.claude/skills/repo-skill/SKILL.md')).exists()
  ).toBe(false);

  const repoDiff = await runSkillsetCli(
    'diff',
    '--root',
    root,
    '--scope',
  ...
  const pluginList = await runSkillsetCli(
    'list',
    '--root',
    root,
    '--scope',
...
`,
  ...
  await buildSkillset(root);
  await writeFile(
    join(root, 'plugins-claude/.skillset.lock'),
    '{ not valid json',
    'utf-8'
  );
  ...
  await expect(
    checkSkillset(root, { scopes: ['repo'] })
  ).resolves.toBeDefined();
  await expect(
    buildSkillset(root, { scopes: ['repo'] })
  ).resolves.toBeDefined();

  const explained = await runSkillsetCli(
    'explain',
    '.claude/skills/repo-skill/SKILL.md',
    '--root',
    root,
    '--scope',
    'repo'
...
`,
  ...
  const claudeMcp = await readFile(
    join(root, 'plugins-claude/plugins/alpha/.mcp.json'),
    'utf-8'
  );
  ...
  const listed = await runSkillsetCli(
    'list',
    '--root',
    root,
    '--scope',
  ...
  const explained = await runSkillsetCli(
    'explain',
    'plugins-claude/plugins/alpha/.mcp.json',
    '--root',
    root
  );
  expect(explained.exitCode).toBe(0);
  ...
  expect(explained.stdout).toContain(
    'source pointer: repo:integrations/alpha-mcp.json'
  );
});
...
`,
  ...
  expect(
    await fileExists(join(root, 'plugins-claude/plugins/alpha/.mcp.json'))
  ).toBe(false);
  expect(
    await fileExists(join(root, 'plugins-codex/plugins/alpha/.mcp.json'))
  ).toBe(false);
});
...
`,
  ...
  expect(
    await fileExists(join(root, 'plugins-claude/plugins/alpha/bin/tool'))
  ).toBe(true);
  const manifest = await readFile(
    join(root, 'plugins-claude/plugins/alpha/.claude-plugin/plugin.json'),
    'utf-8'
...
`,
  ...
  await expect(buildSkillset(generatedRoot)).rejects.toThrow(
    'inside generated output root outputs.plugins.claude'
  );
...
`,
  ...
  await expect(buildSkillset(missingRoot)).rejects.toThrow(
    'points to missing path repo:missing/mcp.json'
  );
});
...
`,
  ...
  await expect(buildSkillset(mcpDirectoryRoot)).rejects.toThrow(
    'feature mcp source must be a file'
  );
...
`,
  ...
  await expect(buildSkillset(binFileRoot)).rejects.toThrow(
    'feature bin source must be a directory'
  );
});
...
test('SET-27: init previews by default and writes only with confirmation', async () => {
  ...
  expect(written.exitCode).toBe(0);
  const config = await readFile(join(root, '.skillset/config.yaml'), 'utf-8');
...
test('SET-27: init scaffolds optional includes only when requested', async () => {
  ...
  expect(await fileExists(join(root, '.skillset/src/agents/.gitkeep'))).toBe(
    false
  );
  expect(
    await fileExists(join(root, '.github/workflows/skillset-ci.yml'))
  ).toBe(false);
  ...
  expect(await fileExists(join(shaped, '.skillset/src/agents/.gitkeep'))).toBe(
    true
  );
  expect(
    await fileExists(join(shaped, '.github/workflows/skillset-ci.yml'))
  ).toBe(true);
});
...
test('SET-27: create makes a new source repo with default naming', async () => {
  ...
  expect(
    await fileExists(join(parent, 'my-skillset/.skillset/config.yaml'))
  ).toBe(false);

  const written = await runSkillsetCli('create', '--root', parent, '--yes');
  expect(written.exitCode).toBe(0);
  const config = await readFile(
    join(parent, 'my-skillset/.skillset/config.yaml'),
    'utf-8'
  );
...
test('SET-27: create supports global source path without touching runtime config', async () => {
  ...
  expect(
    await fileExists(join(home, '.skillset/src/.skillset/config.yaml'))
  ).toBe(true);
...
test('SET-27: setup refuses unsafe overwrite', async () => {
  ...
  const create = await runSkillsetCli(
    'create',
    'occupied',
    '--root',
    parent,
...
`,
  ...
  const initialized = await runSkillsetCliIn(
    join(root, 'nested'),
    'init',
    '--yes'
  );
  ...
  expect(state.scopes['config:root']?.sourceHash).toBe(
    sourceInventoryUnit(inventory, 'config:root').hash
  );
  expect(state.scopes['skill:demo']?.version).toBe('2.3.4');
  expect(state.scopes['skill:demo']?.sourceHash).toBe(
    sourceInventoryUnit(inventory, 'skill:demo').hash
  );
  expect(await fileExists(join(root, '.skillset/changes/history.jsonl'))).toBe(
    false
  );
  expect(await fileExists(join(root, '.skillset/changes/releases.jsonl'))).toBe(
    false
  );
});
...
`,
  ...
  await expect(
    readFile(join(root, '.skillset/changes/state.json'), 'utf-8')
...
`,
  ...
  const releasedState = JSON.parse(await readFile(statePath, 'utf-8')) as {
    scopes: Record<string, { version: string }>;
  };
  const releasedSkillState = releasedState.scopes['skill:demo'];
  if (releasedSkillState === undefined) {
    throw new Error('missing released skill state for skill:demo');
  }
  releasedSkillState.version = '0.6.0';
  await writeFile(
    statePath,
...
`,
  ...
  expect(preview.stdout).not.toContain(
    '? import candidate skills .agents/skills'
  );
  expect(preview.stdout).not.toContain(
    '? import candidate plugins plugins-codex/plugins'
  );
});
...
`,
  ...
  expect(initialized.stderr).toContain(
    'release baseline conflicts with existing release state'
  );
...
`
  ...
  expect(
    report.baselines.map((entry) => [entry.scope, entry.status, entry.version])
  ).toEqual([['skill:adopted', 'create', '3.4.5']]);

  const inventory = await collectSourceInventory(root);
  const state = await readReleaseState(root);
  expect(state.scopes['skill:adopted']?.version).toBe('3.4.5');
  expect(state.scopes['skill:adopted']?.sourceHash).toBe(
    sourceInventoryUnit(inventory, 'skill:adopted').hash
  );
  expect(await fileExists(join(root, '.skillset/changes/history.jsonl'))).toBe(
    false
  );
...
`
  ...
  await Bun.write(
    join(external, 'skills/demo/SKILL.md'),
    `---
name: demo
description: Demo.
---
...
`
  ...
  const state = await readReleaseState(root);
  for (const scope of [
    'plugin.widget.config:root',
    'plugin.widget.skill:demo',
    'plugin:widget',
  ]) {
    expect(state.scopes[scope]?.version).toBe('0.8.0');
    expect(state.scopes[scope]?.sourceHash).toBe(
      sourceInventoryUnit(inventory, scope).hash
    );
  }
});
...
test('SET-27: setup-only flags fail loudly outside their setup command', async () => {
  ...
  expect(buildTargets.stderr).toContain(
    'setup options are only supported with init or create'
  );

  const createGlobalPath = await runSkillsetCli(
    'create',
    'team-loadout',
    '--global'
  );
...
`,
  ...
  expect(generated.entries[0]?.sourcePath).toBe(
    '.skillset/skills/demo/SKILL.md'
  );
...
`,
  ...
  expect(
    badReport.lintIssues.some(
      (issue) => issue.code === 'resource-undeclared-link'
    )
  ).toBe(true);
...
`,
  ...
  await expect(lintSkillset(root)).rejects.toThrow(
    'links to undeclared resource shared:references/guide.md'
  );
  await expect(lintSkillset(root)).rejects.toThrow(
    'resources: { references: [shared:references/guide.md] }'
  );
});
...
function sourceInventoryUnit(
  ...
  if (unit === undefined) {
    throw new Error(`missing source inventory unit ${id}`);
  }
...
function extractChangeRef(stdout: string): string {
  ...
  if (match === null) {
    throw new Error(`missing change ref in stdout:\n${stdout}`);
  }
  return match[0];
}
...
async function runSkillsetCliIn(
  cwd: string,
...
async function runSkillsetCliWithInput(
  input: string,
  ...args: readonly string[]
): Promise<{
  readonly exitCode: number;
