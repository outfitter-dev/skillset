...
test('kitchen-sink fixture builds every implemented surface and stays current', async () => {
  ...
  expect(
    await readFile(
      join(
        root,
        'plugins-claude/plugins/kitchen/skills/sink/references/shared-ref.md'
      ),
      'utf-8'
    )
  ...
  expect(
    await readFile(
      join(
        root,
        'plugins-codex/plugins/kitchen/skills/sink/references/plugin-ref.md'
      ),
      'utf-8'
    )
  ...
  expect(
    await readFile(
      join(root, 'plugins-claude/plugins/kitchen/skills/sink/docs/report.md'),
      'utf-8'
    )
  ...
  expect(codexSkill).toContain(
    '[plugin reference](references/plugin-ref.md#usage)'
  );
  ...
  expect(
    await exists(
      join(root, 'plugins-claude/plugins/kitchen/commands/review.md')
    )
  ).toBe(true);
  expect(
    await exists(join(root, 'plugins-claude/plugins/kitchen/hooks/hooks.json'))
  ).toBe(true);
  expect(
    await exists(join(root, 'plugins-claude/plugins/kitchen/.mcp.json'))
  ).toBe(true);
  // SET-2: Codex hooks emit at the documented hooks/hooks.json path.
  expect(
    await exists(join(root, 'plugins-codex/plugins/kitchen/hooks.json'))
  ).toBe(false);
  ...
  expect(
    await exists(join(root, 'plugins-codex/plugins/kitchen/.app.json'))
  ).toBe(true);
  expect(
    await exists(join(root, 'plugins-codex/plugins/kitchen/.mcp.json'))
  ).toBe(true);
  // SET-8: Claude-native pass-through surfaces are copied and declared in the manifest.
  expect(
    await exists(join(root, 'plugins-claude/plugins/kitchen/.lsp.json'))
  ).toBe(true);
  expect(
    await exists(
      join(root, 'plugins-claude/plugins/kitchen/output-styles/concise.md')
    )
  ).toBe(true);
  expect(
    await exists(
      join(root, 'plugins-claude/plugins/kitchen/themes/midnight.json')
    )
  ).toBe(true);
  expect(
    await exists(
      join(root, 'plugins-claude/plugins/kitchen/monitors/monitors.json')
    )
  ).toBe(true);
  ...
  expect(claudeKitchenManifest).toContain(
    `"monitors": "./monitors/monitors.json"`
  );
  // SET-8: these Claude-native surfaces are not copied into Codex output.
  expect(
    await exists(join(root, 'plugins-codex/plugins/kitchen/.lsp.json'))
  ).toBe(false);
  expect(
    await exists(
      join(root, 'plugins-codex/plugins/kitchen/themes/midnight.json')
    )
  ).toBe(false);
  // Claude agents/ surface absent from Codex output (none declared here either).
  expect(
    await exists(join(root, 'plugins-codex/plugins/kitchen/commands/review.md'))
  ).toBe(false);
  ...
  ).toContain('Global Rule');
  const docsRule = await readFile(
  ...
  expect(await readFile(join(root, 'AGENTS.md'), 'utf-8')).toContain(
    'Build from . and inspect'
  );
  expect(await readFile(join(root, 'docs/AGENTS.md'), 'utf-8')).toContain(
    'lives under docs'
  );
...
`,
  ...
  await expect(buildSkillset(root)).rejects.toThrow(
    'only runs type: command hook handlers'
  );
...
`,
  ...
  await buildSkillset(root);
  await writeFile(join(root, '.skillset.lock'), '{ not valid json', 'utf-8');

  await expect(checkSkillset(root)).rejects.toThrow(
    'cannot guard generated state'
  );
  await expect(buildSkillset(root)).rejects.toThrow(
    'cannot guard generated state'
  );
...
`,
  ...
  await expect(checkSkillset(root)).rejects.toThrow(
    'generated lock plugins-claude/.skillset.lock cannot guard generated state'
  );
  await expect(buildSkillset(root)).rejects.toThrow(
    'generated lock plugins-claude/.skillset.lock cannot guard generated state'
  );
...
test('compareStrings orders by code unit independent of locale', () => {
  ...
  const sorted = [...input].toSorted(compareStrings);
