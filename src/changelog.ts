...
import { readAppliedChangeRecords, groupRef } from './change-workflow';
import type { AppliedChangeRecord } from './change-workflow';
...
export async function renderChangelogProjections(
  graph: BuildGraph
): Promise<readonly ChangelogProjection[]> {
  const changes = (
    await readAppliedChangeRecords(graph.rootPath, {
  ...
  for (const skill of graph.standaloneSkills) {
    const scoped = changesForScopes(changes, [
...
function renderSkillChangelog(
  ...
  const outputPath = join(
    dirname(relative(graph.rootPath, skill.sourcePath)),
    'CHANGELOG.md'
  );
  return projection(
    'skill',
...
function renderPluginChangelog(
  ...
  const outputPath = join(
    relative(graph.rootPath, plugin.path),
    'CHANGELOG.md'
  );
  return projection(
    'plugin',
...
function renderChangeSection(change: AppliedChangeRecord): string {
  ...
  if (change.scopes.length > 0) {
    details.push(`scopes: ${sourceUnitDisplays(change.scopes)}`);
  }
  if (details.length > 0) {
...
function changesForPlugin(
  ...
  return changes
    .filter((change) =>
      change.scopes.some((scope) => isPluginChangelogScope(scope, plugin.id))
    )
    .toSorted(compareChangeRecords);
}
...
function compareChangeRecords(
  left: AppliedChangeRecord,
  right: AppliedChangeRecord
): number {
...
function hashChanges(
  ...
  for (const change of changes) {
    ...
    hash.update('\0');
    for (const scope of [...change.scopes].toSorted(compareStrings)) {
      hash.update('scope\0');
      hash.update(scope);
      hash.update('\0');
      for (const sourceHash of [
        ...(change.sourceHashes.get(scope) ?? []),
