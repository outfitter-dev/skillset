...
Propagation polling backs off from 3 seconds to a maximum 30-second interval over a five-minute wait per package. Release planning and preflight also wait for complete metadata on occupied versions before applying strict release-set checks; absent versions remain missing without a propagation wait. Before each publish attempt it checks the registry again and skips occupied versions. If npm rejects an attempt with the exact target version already occupied, the publisher waits for matching staged integrity, required provenance, and the intended dist-tag before continuing; the error text alone is not success. Other npm errors fail immediately. A propagation timeout stops the set and explains that publication may already have succeeded: inspect registry state and retry through the protected release workflow, without manually republishing.
