...
The publisher uses npm 11.12.1 to stage all seven tarballs before the first registry mutation. It records each tarball's SHA-512 integrity, publishes the five native packages, then `@skillset/cli`, then `skillset`, and waits after every package for the exact version, dist-tag, integrity, and SLSA provenance to appear. It rechecks all six prerequisites immediately before the launcher.

Propagation polling backs off from 3 seconds to a maximum 30-second interval over a five-minute wait per package. Release planning and preflight also wait for complete metadata on occupied versions before applying strict release-set checks; an absent suffix remains missing without a propagation wait. An absent package before a later published package is a possible stale registry read, so planning polls that gap with the same bounded verification before applying the strict prefix check. A gap that stays absent times out without publishing. Before each publish attempt it checks the registry again and skips occupied versions. If npm rejects an attempt with the exact target version already occupied, the publisher waits for matching staged integrity, required provenance, and the intended dist-tag before continuing; the error text alone is not success. Other npm errors fail immediately. A propagation timeout stops the set and explains that publication may already have succeeded: inspect registry state and retry through the protected release workflow, without manually republishing.

A partial prefix may resume through the manual environment while the launcher is absent. A non-prefix state, missing provenance, immutable tarball mismatch, dist-tag drift, or a launcher published before a prerequisite blocks and requires a new version instead of silent repair.
...
The handoff downloads the exact seven GitHub release assets, verifies their GitHub attestations, checks the `SHA256SUMS` inventory and bytes, and validates the native manifest's exact five-target metadata. It then renders a macOS-only formula from the verified Darwin checksums and keeps the tap README Skillset section aligned with the tap's `brew pr-pull` landing contract. The workflow opens or updates a draft `release/skillset` pull request in `outfitter-dev/homebrew-tap`; it never marks that pull request ready, never GitHub-merges it, and never publishes bottles. Tap-owned CI must pass strict audit plus install and test on Apple Silicon and Intel macOS. After those checks pass, a maintainer dispatches the tap `brew pr-pull` workflow from `main` with the draft pull request number and its exact tested head SHA. `brew pr-pull` owns both landing the formula and publishing its bottles.
...
For recovery after a successful npm and GitHub release, manually dispatch `Publish Homebrew` with the current published stable tag. The workflow serializes every Skillset handoff through one concurrency group and reuses one tap branch, so a newer stable release updates the existing draft proposal instead of creating mergeable stale-version branches. A missing token, non-latest tag, asset mismatch, invalid manifest, failed attestation, unmanaged tap README section, or tap PR conflict stops without landing. Resolve the evidence or branch conflict, then rerun the protected workflow; do not hand-edit generated checksums, GitHub-merge the formula pull request, or bypass tap CI.
...
`scripts/native-size-baseline.json` records Bun 1.4.0 measurements for every required target and may retain informational measurements for buildable reserved targets. A target may grow by 10% or 2 MiB, whichever is larger, before verification fails. That budget catches an embedded-runtime or packaging regression without making ordinary compiler growth churn the baseline. Verification accepts the baseline while both its recorded Bun version and the building Bun fall inside `package.json#engines.bun`, compared on the numeric base version so a canary such as `1.4.1-canary.20` counts as `1.4.1`. Moving `.bun-version` within that range, such as from 1.4.0 to 1.4.2, therefore keeps the existing baseline; the growth budget is what flags a runtime that changed the embedded size. Changing `engines.bun` so the recorded version falls outside it fails verification and requires explicit remeasurement and review. The range must set a lower bound: `bun run check` rejects an empty `engines.bun` or one that admits `0.0.0`, which includes any unparseable range, because Bun's semver matcher treats those as matching every version.
