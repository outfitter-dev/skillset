...
Feature narratives remain authored pages. Standardized public feature pages use the generic `feature-support` generated block, which `docs:generate` rewrites from the Core registry while preserving surrounding prose. Development feature pages retain the legacy Core-checked feature-support block until that cohort is standardized. The registry's `docs` references are the canonical linkage between a feature and one or more authored pages. Introductory pages under `start/`, `guides/`, and the repository front door link to those facts but do not acquire registry entries simply to satisfy navigation.
