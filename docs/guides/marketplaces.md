...
```bash
bunx @skillset/cli marketplace check
bunx @skillset/cli marketplace check outfitter --json
```
...
```bash
bunx @skillset/cli marketplace update outfitter --json
```

Then authorize the reviewed repository writes:

```bash
bunx @skillset/cli marketplace update outfitter --yes
```
