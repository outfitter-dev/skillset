...
Maintainers refresh that evidence explicitly from this repository with `bun run providers:check`, `bun run providers:diff`, and `bun run providers:update`. Check compares adopted schema source hashes against live upstream sources, diff adds readable schema summary changes plus explicit manual-review rows for prose-only destination format snapshots, and update rewrites `packages/registry/src/schema-snapshots.ts` with refreshed schema summaries and provenance. Destination format snapshots are not machine-diffed until their source rows carry a machine-readable upstream baseline. This machinery is not shipped as a public CLI command, and ordinary user builds and checks remain network-free.
...
`skillset check` reports generated-output drift that maps to safe provider destination-format migrations without writing. `skillset check --ci --fix`, local `skillset check --write`, and `skillset update --yes` apply only source-preserving safe plans; unregistered or manual-review drift blocks writes and prints the affected outputs.
