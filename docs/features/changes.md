...
Existing YAML-frontmatter pending entries remain a compatibility input during the cutover. Compatibility validation still uses the generated `@skillset/schema` change-entry contract before applying semantic checks such as duplicate ids, known scopes, evidence freshness, and source coverage. The schema reference and examples are retired in the follow-up cleanup slice once migration docs land.
