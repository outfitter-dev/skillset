...
`PostToolUse` is advisory: after write/edit tools it runs `skillset change status --root .` only when one of those paths has a tracked or untracked change, and it does not block the agent turn. `Stop` is blocking but uses the same path gate before running `skillset change check --root .`, `skillset check --root .`, and `skillset check --only outputs --root .`. `Stop` deliberately does not run the broader `bootstrap.sh doctor` environment diagnostics; explicit bootstrap diagnostics and pre-push snippets remain the broader guardrail. Runtime suggestions remain opt-in reviewed configuration, and the public snippets call the installable `skillset hooks run ...` commands.
