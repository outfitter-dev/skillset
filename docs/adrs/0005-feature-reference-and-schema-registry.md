...
# ADR-0005: Feature Reference and Schema Registry

## Context

Skillset covers a broad and growing source surface: skills, plugin manifests, instructions, hooks, MCP servers, apps, resources, skill tools policy, agents, commands, executables, themes, monitors, LSP servers, supports metadata, dependencies, distributions, tests, release state, target-native islands, and future runtime adapters. The compiler cannot keep that surface honest with prose alone.

The pressure is not just documentation completeness. A feature can drift in several ways:

- The docs say a source feature renders to a target, but the compiler does not.
- The compiler supports a target feature, but the docs do not explain the target nuance or unsupported cases.
- A near-match feature is accidentally treated as portable because Claude, Codex, and Cursor use similar words for different behavior.
...
Skillset uses a typed feature registry that is intentionally smaller than a public plugin system and stricter than a hand-written support matrix. It is the internal authority for feature capability claims, docs ownership, adapter evidence, validation ownership, and conformance checks.
...
The registry is the typed version of Skillset's support matrix. Each entry names a feature, its source shape, current maturity, target support, evidence, docs, render owner, and validation owner.

The registry is allowed to answer:

- What stable feature id should docs, diagnostics, locks, Render Results, reports, and tests use?
- Is the feature implemented, planned, reserved, deferred, future-only, or unsupported as a Skillset source feature?
- Can Claude, Codex, and Cursor represent this feature, and by what support shape?
- Which source file or module owns derivation or rendering?
...
Render Result statuses are separate. A target support row that says `native` usually produces a `rendered` result when source is built. A target support row that says `pass_through` usually produces a `target_native` result. A target support row that says `unsupported` may produce `unsupported`, `intentionally_skipped`, or no result when the source unit is not in scope.
...
The core representation is the resolved source graph plus typed feature entries, target support rows, Render Results, diagnostics, locks, and operation results. Those facts can serialize to Markdown, JSON, YAML, TOML, or target-specific directories, but the serialization does not define the semantic contract.

The test: if changing a generated filename would not change the author's source intent, the filename is not the IR. If changing a source feature row would change validation, rendering, or support claims, that row is part of the contract.
...
- `source`: compiler modules that parse, validate, derive, render, or report the feature.
...
External provider docs prove that a target surface exists; they do not prove Skillset's rendering is correct. Source and tests prove Skillset behavior; they do not prove provider runtime activation. Runtime support and activation probes must stay separate from compile-target support.

### Boundary With Adapters

The registry describes capabilities across adapters, but it is not a public adapter API. Claude, Codex, and Cursor adapter code can evolve internally, and future targets can add registry rows or runtime support records without making adopters learn a new plugin system.
...
Conformance checks consume the registry and Render Results together. A registry row that says `degraded` produces a visible degraded result when that source feature is built for the target. A row that says `unsupported` produces an unsupported result, a clear render error, or an intentionally skipped result with policy provenance. Silent omission is not a valid conformance result.
...
The registry gives future implementation branches a stable place to attach diagnostics, drift checks, feature inspection, Render Results, and conformance fixtures. It also gives docs a way to say "this is target-native" or "this is degraded" without burying that truth in a prose paragraph.

Separating capability status from Render Results keeps build reports honest. A feature can be generally supported while a particular source unit is skipped by scope, and a feature can be generally degraded without every build producing degraded output.
...
A registry can become stale if checks do not verify references and evidence. The
implemented registry drift check validates docs, tests, fixtures, and support
claims against owned evidence.

Registry ids can make diagnostics look machine-first. Current diagnostics keep
human-readable messages primary and add feature ids as structured context, not
as replacements for explanations.

## Acceptance Evidence (2026-07-20)

The registry is implemented in
`packages/core/src/feature-registry.ts`; targets are derived from the canonical
schema registry and include Claude, Codex, and Cursor. Each entry owns source
shape, maturity, per-target support, evidence, documentation, render ownership,
and validation ownership. Render Results, not registry capability rows, record what a
particular build rendered, transformed, degraded, skipped, or rejected.

`checkFeatureRegistryDrift` validates evidence links and generated support
tables. Representative adapter-conformance coverage reports missing, stale,
invalid, and unsupported-without-fixture gaps. These checks prove compiler
claims and fixture coverage; they do not claim exhaustive provider runtime
