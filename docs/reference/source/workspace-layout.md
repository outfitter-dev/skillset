...
Applicable adaptive source inherently renders adopted Agent standards: instructions become root or scoped `AGENTS.md`, skills become `.agents/skills/<skill>/`, and portable plugins become `plugins/<plugin>/agents/`. An enabled [target](../../glossary.md#target) separately receives files in its native repository layout. Claude and Cursor use their native skill and rule roots; Codex can share compatible standard-owned files and adds project configuration under `.codex/`. Provider plugin bundles normally render below `plugins/<plugin>/claude/`, `plugins/<plugin>/chatgpt/` for the Codex-selected product bundle, or `plugins/<plugin>/cursor/`. Configuration can select or relocate supported provider destinations, but it does not disable or relocate the standards floor.
