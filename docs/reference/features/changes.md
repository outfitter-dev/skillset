...
`union` keeps both sides' appended records without a conflict, and being built in it needs no per-clone `git config` registration, so it applies the same way in CI, fresh clones, and worktrees. Restack (rebase) a branch onto trunk to update it: union then replays the branch's appended block after trunk's records. Merging trunk into a branch (`git merge main`, GitHub's "Update branch") does the opposite — trunk's new records land after the branch's — and the guard below rejects that result by design.

Union is line-level and cannot validate the result. Two invariants still need checking after a merge: record ids stay unique, and trunk's records keep their order. File order is the only fold order: derived state walks records top to bottom and later records win. `createdAt`, `appliedAt`, and `amendedAt` are event time, not sequence, so global chronological order is not an invariant. `merge=union` can place an older appended block after a newer trunk tail; that is fine. What is not fine is changing what trunk already has, so `bun run change-stream:guard` reads each stream at the merge-base with the remote trunk and requires those lines to be an unchanged, in-order prefix of the working copy, naming the first divergent line when they are not. Live writers serialize on the owner-fenced ledger lock and stamp `max(now, tail)`, so event time stays non-decreasing in file order for readers of live appends. Never sort a stream to fix a failure; restore trunk's records and move the branch's records after them. See [ADR-0038](../../adrs/0038-change-ledger-append-order.md).
