...
`skillset import --from <provider>` reverses provider transport, header, placeholder, and filename spellings into canonical `.mcp.json` before committing imported source. Importing an Agent Plugins package uses `--from agents`; here `agents` identifies the standards origin, not a provider.
