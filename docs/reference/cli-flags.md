...
| `status` | `--root`, `--activation`, `--json` | Read-only human health/advisory view. `--activation` explicitly adds bounded local provider observation without changing the health exit contract. |
| `list` | `--root`, `--scope`, `--json` | Inventory, not build-mode selection. |
| `explain <path>` | `--root`, `--scope`, `--activation`, `--json` | Workspace-specific provenance. `--activation` adds only observations whose requirements are owned by the explained path. |
| `lookup` | `--frontmatter`, `--fields`, `--field`, `--values`, `--events`, `--compat`, `--examples`, `--schema`, `--json` | Workspace-independent reference lookup; `--root` is rejected. `lookup activation` reports Core-owned activation policy joined to Registry provider evidence without reading local runtime state. A bare or incomplete TTY request guides missing subject/view choices from Core-owned reports and registries, then exits after one ordinary report. Explicit flags, JSON, pipes, and non-TTY execution do not prompt. Provider aliases become values of `--compat`, not separate `--claude`/`--codex`/`--cursor` flags. |
