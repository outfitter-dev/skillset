...
Project agents render as Claude Markdown under `.claude/agents/`. Claude-enabled adaptive instructions without `paths` aggregate into root `CLAUDE.md`; path-scoped instructions render under `.claude/rules/` with native `paths` frontmatter. Each instruction has one Claude destination, and existing unmanaged guidance remains protected by output collision checks. Provider-native files remain separate from adaptive source so Claude-specific semantics are visible rather than presented as portable.
