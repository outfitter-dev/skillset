...
When `compile.targets` is omitted, Skillset normalizes to the default provider plan, currently Claude and Codex until Cursor parity is complete. Target-specific provider blocks configure native output details and provider-specific opt-outs; they are not a second target-selection surface.
