...
The managed known-Skillsets index lives at `$XDG_CONFIG_HOME/skillset/skillsets.json`. It records local checkout paths and normalized repo identities, such as `github:owner/repo`, for marketplace repo resolution convenience. It is machine-local config state, not committed source truth; CI and portable marketplace verification must still resolve from committed marketplace source and remote refs. Writes are serialized and atomically replace the prior flushed document. A write-capable command preserves malformed bytes in a unique `skillsets.corrupt-*.json` sibling before rebuilding this disposable state; read-only lookup remains non-mutating. Successful registrations advance an optional schema-v1 maintenance cursor and inspect no more than 128 path-sorted entries, pruning only confirmed missing paths and non-directories. Live and ambiguously inspected paths are retained, and deleting the disposable index is safe because later workspace commands repopulate it.

The draft global-workspace contract reserves
`$XDG_DATA_HOME/skillset/source/` for a root `skillset.yaml` and the normal flat
`.skillset/` source tree, `$XDG_DATA_HOME/skillset/build/` for generated
projection, `$XDG_CONFIG_HOME/skillset/config.yaml` for Skillset preferences,
and `$XDG_STATE_HOME/skillset/activations.json` for future provider-specific
activation ownership. These paths are not implemented as a public user-scope
workflow: current commands remain repo-local by default, honor only their
existing explicit root selection, and never fall back to a global workspace.
ADR-0008 historically proposed `~/.skillset/src`; its nested `.skillset/src`
workspace topology is retired and rejected by current source loading. The later
`~/.skillset/source` default belongs only to an internal setup API. Neither path
is canonical, auto-discovered, or migrated. Future install/sync work must be
preview-first, require `--yes`, refuse collisions or tampered ledger entries,
reverse only Skillset-owned activations, and never edit provider settings or
trust. See the [global/XDG draft ADR](adrs/drafts/20260604-global-xdg-managed-installs-and-sync.md).
...
`skillset create [name]` creates a named child under the current directory or explicit `--root` parent. It writes root `skillset.yaml`, `.skillset/` source-family placeholders, repo guidance, and a local Git repository. `skillset init [directory]` accepts only an existing directory, preserves unrelated files, and plans only missing Skillset setup. Neither command creates or mutates user-level runtime configuration, marketplaces, trust settings, or symlinks.

`skillset new` scaffolds source units inside `.skillset/`. It is preview-first like setup and build: pass `--yes` to write. `skillset new skill "Docs CLI Expert"` derives `docs-cli-expert` and writes `.skillset/skills/docs-cli-expert/SKILL.md`; `skillset new instruction "Review Guidance"` writes `.skillset/rules/review-guidance.md`. Use `--id <id>` and `--name <display name>` when stable identity and display text differ. Use `--in <plugin-name>` to place a skill or instruction under an existing plugin container; missing containers fail instead of creating implicit plugins. `--preset support` adds empty `references/`, `assets/`, and `scripts/` directories, `--preset evals` adds a portable `evals/evals.json` with the matching `skill_name`, and `--preset reference-file` or `--preset examples-file` creates single-file support files. Eval declarations are source-only; use `skillset eval list` to inspect their resolved case/target matrix without invoking a provider. `skillset new agent <name>` writes a project-agent source file under `.skillset/agents/`. `skillset new hook` creates a schema-valid adaptive hook beside the existing plugin, skill, or project agent selected by `--attach`, and adds the corresponding `hooks.auto` attachment. Events and compatible providers come from the shared registry. New is preview-first, refuses collisions, and never builds automatically.

Create defaults to every provider returned by the shared target registry and offers an explicit provider picker. The generated setup config uses `compile.targets` for provider selection. Target-native adapter settings belong in explicit provider blocks such as `claude`, `codex`, and `cursor`, and reusable defaults belong in `defaults.<target>.<surface>` or the target-local `defaults` block. The published package requires Bun and ships Bun-built JavaScript bins for `skillset` and `skillset-toolkit`; stable releases run from the default npm dist-tag with commands such as `npx skillset create my-skillset` or `bunx skillset create my-skillset`.
