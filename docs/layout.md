...
`skillset create` creates a new Skillset source repo, defaulting to `my-skillset` under the current directory. Local create writes root `skillset.yaml`, `.skillset/` source-family placeholders, `.skillset/changes/.gitkeep`, root `skillset.lock`, `.skillset/.gitignore`, a `.skillset/snapshots/.gitignore` sentinel, a root `.gitignore` that ignores `.skillset/cache/` and `.skillset/snapshots/` contents, a README, lightweight `AGENTS.md` guidance, and a Git repository by default. `skillset create --global` defaults the source checkout to `~/.skillset/src` and stays source-only: it writes root `skillset.yaml`, `.skillset/` placeholders, and `.skillset/changes/.gitkeep` inside that checkout without creating a Git repo, repo-local guidance, cache output, snapshots, or live runtime config. Setup does not create or mutate `~/.claude`, `~/.codex`, `~/.cursor`, `.agents`, marketplaces, trust settings, or symlinks. Future create work can add starter source templates and reviewed provider configuration suggestions without writing live runtime settings.
