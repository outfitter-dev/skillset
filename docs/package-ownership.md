...
| `config` | 11 / 10 | Setup, import, adoption, change, and test flows consume raw config readers, validators, and target helpers. | Canonical target helpers already have root exports; raw config parsing stays private or moves through `@skillset/schema` by contract. |
...
| `plugin-output` | 1 / 1 | The retained ad hoc test workflow needs the compiler's private plugin destination calculation. | Keep internal as output-layout implementation detail. |
...
| `resolver` | 10 / 10 | Adoption, change, development, import, new-source, release, and test flows still consume raw graph loading or source discovery. | Promote only future operation-shaped contracts with named library consumers; do not expose the raw resolver. |
...
| `structured-output` | 3 / 3 | Retained and declared tests use validated serialization inside app presentation adapters. | Keep internal; CLI serialization use alone is not a public Core contract. |
