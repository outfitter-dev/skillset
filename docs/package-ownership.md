...
| --- | --- | --- |
| `packages/core/src/render.ts` | Provider rendering, target-native serialization, generated lock/provenance files, plugin manifests, rendered skills/rules/islands, adaptive hook rendering, changelog projections, resource link rewriting, and output hashing. | Extract only when a submodule has a clear provider, feature, or destination outcome responsibility. Good candidates are hook rendering, manifest rendering, lock/provenance rendering, or resource-link rewriting if tests can cover them directly. |
