...
const NATIVE_PLUGIN_MANIFEST_DIRS = new Set(targetNames().map((target) => `.${target}-plugin`));
...
async function detectSurveySkips(rootPath: string): Promise<readonly SurveySkip[]> {
  ...
  for (const path of await foreignPluginManifests(rootPath)) {
    const reason = `plugin manifest for an unsupported target; skillset can only represent ${targetNames().join(", ")} surfaces`;
    skips.push({
...
async function hasNativePluginManifest(rootPath: string): Promise<boolean> {
  for (const target of targetNames()) {
    if (await pathExists(join(rootPath, `.${target}-plugin/plugin.json`))) return true;
  }
  return false;
}
...
function createReadme(name: string, targets: readonly TargetName[]): string {
  return [
    `# ${name}`,
    "",
    "This repository is a Skillset source repo. Edit authored source under `.skillset/`, then run Skillset commands to preview or write generated provider outputs.",
    "",
    "## Quick Start",
    "",
    "```bash",
...
function normalizeTargets(targets: readonly TargetName[] | undefined): readonly TargetName[] {
  ...
  for (const target of targets) {
    if (!isTargetName(target)) {
      throw new Error(`skillset: unsupported setup target ${JSON.stringify(target)}; expected ${targetNames().join(", ")}`);
    }
    seen.add(target);
  }
  return [...seen];
