...
import type { BuildGraph, JsonRecord, RuntimeTesterClaudeSettingSources, SkillsetOptions, TargetName } from "./types";
...
export async function startRuntimeTesterRun(
  ...
  const config: RuntimeTesterStoredConfig = {
    ...(options.target === "claude" ? { claudeSettingSources: resolveClaudeSettingSources(graph, options) } : {}),
    name,
  ...
  await writeStatus(paths, {
    latestRoot: latestRootLogicalPath(),
    name,
...
function runtimeCommand(
  ...
  const latestRoot = resolveOperationalPath(runtimePathContext(rootPath, graph, xdg), latestRootLogicalPath());
...
function resolveClaudeSettingSources(
  ...
  return options.claudeSettingSources ??
    ...
    graph.root.workspace.runtimeTester?.claude?.settingSources ??
...
function latestRootLogicalPath(): string {
  return ".skillset/cache/latest";
}
