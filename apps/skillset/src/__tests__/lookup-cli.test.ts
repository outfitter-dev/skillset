...
test("SET-220: lookup hooks events and Codex compatibility", async () => {
  ...
  expect(result.exitCode).toBe(0);
  expect(result.stdout).toContain("[codex] PreToolUse matcher: tool handlers: command");
...
test("SET-220: lookup target lens aliases filter non-compat views", async () => {
  ...
  const report = JSON.parse(result.stdout) as {
    ...
    readonly events: readonly { readonly handlerTypes: readonly string[]; readonly matcherKind: string; readonly matcherValues: readonly string[]; readonly name: string; readonly target: string }[];
