...
test("runtime tester resolves Claude setting sources from config, env, and CLI options", async () => {
  const root = await fixture({
    "skillset.yaml": `
skillset:
...
`,
  ...
  const fromConfig = await startRuntimeTesterRun(root, {
    env: { ...process.env, SKILLSET_RUNTIME_TESTER_CLAUDE_BIN: bin },
    prompt: "Inspect Claude config fixture.",
    target: "claude",
    xdg,
  });
  expect((await readRuntimeTesterStatus(root, fromConfig.runId, { xdg })).command?.join(" ")).toContain("--setting-sources project");
