...
test.skipIf(!existsSync(EXPECT))(
  ...
  async () => {
    const root = await createTestFixtureRoot("skillset-reconcile-pty-");
    const xdgRoot = await createTestFixtureRoot("skillset-reconcile-pty-xdg-");
    await Bun.write(
    ...
    expect(declined.stdout).toContain(`skillset: reconcile ${GENERATED_PATH}`);
    const sourcePreview =
