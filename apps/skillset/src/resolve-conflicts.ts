...
/** Conflict stages, in the order a parseable lock is looked for. */
const LOCK_SOURCE_REVISIONS = [":2:", ":3:", "HEAD:"] as const;
...
async function trackedLockPaths(
  rootPath: string
): Promise<readonly string[]> {
  const listed = await git(rootPath, [
    "ls-files",
...
/**
 * The "ours" side of a conflict, falling back to "theirs" and then `HEAD`.
 * Returns undefined when no side has the path.
 */
async function readConflictStage(
  rootPath: string,
  path: string
): Promise<Uint8Array | undefined> {
  for (const revision of LOCK_SOURCE_REVISIONS) {
    const proc = Bun.spawn({
...
async function readHandEditedPaths(
  ...
  const conflicted = new Set(generatedPaths.filter((path) => !isLockPath(path)));
  ...
  for (const stage of CONFLICT_STAGES) {
    ...
    for (const path of payloadPaths) {
      // A conflicted path exists only at stages 2 and 3; an unconflicted
      // sibling exists only at stage 0 and is identical on both sides.
      const readStage = conflicted.has(path) ? stage : 0;
      const content = await readStageBlob(rootPath, readStage, path);
      if (content === undefined) continue;
      const mode = modes.get(path)?.get(readStage) ?? modes.get(path)?.get(0);
      snapshots.set(path, {
        content,
        mode: normalizeGeneratedFileMode(mode ?? 0o644),
      });
    }
...
async function readJsonSafely(file: ReturnType<typeof Bun.file>): Promise<unknown> {
  try {
...
/**
 * Materialize each conflicted lock from the side that claims more output.
 *
 * The lock is regenerated by the repair regardless, so its only job in the
 * meantime is to tell the build which on-disk files it owns. A side that
 * deleted a source unit claims fewer paths, and the survivors would then look
 * like foreign files the build refuses to overwrite. Provenance is a property
...
async function materializeConflictedLocks(
  ...
  for (const lockPath of conflictedLocks) {
    let best: { readonly claims: number; readonly stage: number } | undefined;
    for (const stage of CONFLICT_STAGES) {
      const lockJson = await readStageJson(rootPath, stage, lockPath);
      if (lockJson === undefined) continue;
    ...
    if (best === undefined) {
      await materializeConflictedPaths(rootPath, [lockPath]);
      continue;
    }
    const content = await readStageBlob(rootPath, best.stage, lockPath);
    if (content === undefined) continue;
    const absolute = join(rootPath, lockPath);
    await mkdir(dirname(absolute), { recursive: true });
    await writeFile(absolute, content);
  }
}
