...
import type { BuildScope, CompileBuildMode, JsonRecord, RuntimeTesterClaudeSettingSources, SkillsetOptions, SourceOrigin, TargetName } from "./types";

type Command = "adopt" | "build" | "change" | "check" | "ci" | "create" | "dev" | "diff" | "distribute" | "doctor" | "explain" | "features" | "hooks" | "import" | "init" | "lint" | "list" | "new" | "providers" | "release" | "restore" | "runtime-tester" | "suggest-source" | "test" | "update" | "verify";
  ...
  if (command === "runtime-tester") {
    if (runtimeTesterSubcommand === "run") {
      const report = await startRuntimeTesterRun(rootPath, {
        ...
        target: runtimeTesterTarget ?? "codex",
...
function parseArgs(args: readonly string[]): ParsedArgs {
  ...
  if (
    ...
    throw new Error(
        "skillset: expected command adopt, build, change, check, ci, create, dev, diff, distribute, doctor, explain, features, hooks, import, init, lint, list, new, providers, release, restore, runtime-tester, suggest-source, test, update, or verify\n" +
        USAGE
    );
  }
...
function validateJsonFlags(command: Command, jsonOutput: boolean): void {
  ...
  if (command === "doctor" || command === "explain" || command === "features" || command === "runtime-tester") return;
  throw new Error("skillset: --json is only supported with doctor, explain, or features; runtime-tester also supports --json");
}
