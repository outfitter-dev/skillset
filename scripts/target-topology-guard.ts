...
export const TARGET_TOPOLOGY_ALLOWLIST: readonly TargetTopologyAllowlistEntry[] = [
  allow("packages/schema/src/contracts.ts", 25, 29, "TARGET_NAMES", "R1", '["claude", "codex", "cursor"]', "Canonical target registry declaration."),
  ...
  allow("packages/schema/src/examples.ts", 439, 18, "skillsetSchemaExamples", "R1", '["claude", "codex"]', "Schema example documents an intentionally provider-scoped hook."),
  allow("packages/schema/src/examples.ts", 487, 32, "skillsetSchemaExamples", "R1", '["claude", "codex"]', "Skill eval example demonstrates an intentionally narrowed case target matrix."),
  allow("packages/schema/src/examples.ts", 527, 16, "skillsetSchemaExamples", "R1", '["claude", "codex", "cursor"]', "Schema example demonstrates the complete activation targets field."),
  ...
  allow("packages/core/src/render-result-collector.ts", 1000, 10, "companionForPath", "R2", 'target === "claude" || target === "cursor"', "Commands are provider-native companion formats for Claude and Cursor."),
  allow("packages/core/src/render-result-collector.ts", 1006, 10, "companionForPath", "R2", 'target === "claude" || target === "cursor"', "Agents are provider-native companion formats for Claude and Cursor."),
  allow("packages/core/src/render.ts", 1393, 7, "copyPluginCompanionFiles", "R2", 'target === "codex" || target === "cursor"', "Codex and Cursor hooks require normalized provider-native output."),
  allow("packages/core/src/render.ts", 1413, 10, "copyPluginCompanionFiles", "R2", 'target === "codex" || target === "cursor"', "Codex and Cursor skip copying Claude-native hook files."),
  ...
  allow("packages/core/src/provider-format-conformance.ts", 579, 5, "checkSkillMarkdown", "R3", 'target === "codex" -> target === "cursor" -> else [claude]', "Provider-native skill formats use distinct registry references."),
  allow("packages/core/src/render-plugin-manifest.ts", 82, 5, "renderPluginManifest", "R3", 'target === "claude" -> target === "codex" -> else [cursor]', "Provider-native plugin manifests have distinct formats."),
  allow("packages/core/src/render-plugin-manifest.ts", 242, 3, "withOptionalSurfacePaths", "R3", 'target === "claude" -> target === "codex" -> else [cursor]', "Provider-native plugin surfaces have distinct destination fields."),
  allow("packages/core/src/render.ts", 1375, 5, "copyPluginCompanionFiles", "R3", 'target === "claude" -> target === "codex" -> else [cursor]', "Provider-native companion file sets are intentionally distinct."),
  allow("packages/core/src/render.ts", 307, 3, "marketplaceReadmeLines", "R3", 'target === "claude" -> target === "cursor" -> else [codex]', "Provider-native marketplace README guidance has distinct destination formats."),
] as const;
