...
export const TARGET_TOPOLOGY_ALLOWLIST: readonly TargetTopologyAllowlistEntry[] = [
  ...
  allow("packages/core/src/render-result-collector.ts", 1257, 10, "companionForPath", "R2", 'target === "claude" || target === "cursor"', "Commands are provider-native companion formats for Claude and Cursor."),
  allow("packages/core/src/render-result-collector.ts", 1263, 10, "companionForPath", "R2", 'target === "claude" || target === "cursor"', "Agents are provider-native companion formats for Claude and Cursor."),
  allow("packages/core/src/render-result-collector.ts", 602, 5, "pluginAuthorRenderFacts", "R3", 'target === "claude" -> target === "codex" -> else [cursor]', "Provider-native author omission diagnostics use provider names."),
  allow("packages/core/src/render-result-collector.ts", 611, 11, "pluginAuthorRenderFacts", "R3", 'target === "claude" -> target === "codex" -> else [cursor]', "Provider-native author omission diagnostics use distinct stable codes."),
  ...
  allow("packages/core/src/provider-format-conformance.ts", 540, 5, "checkSkillMarkdown", "R3", 'target === "codex" -> target === "cursor" -> else [claude]', "Provider-native skill formats use distinct registry references."),
