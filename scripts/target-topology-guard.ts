...
export const TARGET_TOPOLOGY_ALLOWLIST: readonly TargetTopologyAllowlistEntry[] = [
  ...
  allow("packages/core/src/render.ts", 1364, 7, "copyPluginCompanionFiles", "R2", 'target === "codex" || target === "cursor"', "Codex and Cursor hooks require normalized provider-native output."),
  allow("packages/core/src/render.ts", 1384, 10, "copyPluginCompanionFiles", "R2", 'target === "codex" || target === "cursor"', "Codex and Cursor skip copying Claude-native hook files."),
  ...
  allow("packages/core/src/render.ts", 1346, 5, "copyPluginCompanionFiles", "R3", 'target === "claude" -> target === "codex" -> else [cursor]', "Provider-native companion file sets are intentionally distinct."),
  allow("packages/core/src/render.ts", 278, 3, "marketplaceReadmeLines", "R3", 'target === "claude" -> target === "cursor" -> else [codex]', "Provider-native marketplace README guidance has distinct destination formats."),
] as const;
