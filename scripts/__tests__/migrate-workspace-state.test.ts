...
describe("workspace state migration", () => {
  it("combines ordinary workspace config and source manifest", async () => {
    const root = await createTestFixtureRoot("skillset-workspace-state-");
...
`,
    ...
    const migrated = parseYamlRecord(
      await readFile(join(root, "skillset.yaml"), "utf8"),
      "workspace"
    );
    ...
    await expect(
      readFile(join(root, ".skillset", "config.yaml"), "utf8")
    ).rejects.toThrow();
    await expect(
      readFile(join(root, ".skillset", "src", "skillset.yaml"), "utf8")
    ).rejects.toThrow();
  });

  it("flattens pending entries and preserves ledger files", async () => {
    const root = await createTestFixtureRoot("skillset-workspace-state-");
    await mkdir(join(root, ".skillset", "changes", "pending"), {
      recursive: true,
    });
    await writeFile(
      join(root, ".skillset", "changes", "pending", "abc123def456.md"),
      "pending\n",
      "utf8"
    );
    await writeFile(
    ...
    expect(result.stderr).toContain(
      "move .skillset/changes/pending/abc123def456.md -> .skillset/changes/abc123def456.md"
    );
    expect(result.stderr).toContain("remove .skillset/changes/pending");
    await expect(
      readFile(join(root, ".skillset", "changes", "abc123def456.md"), "utf8")
    ).resolves.toBe("pending\n");
    await expect(
      readFile(join(root, ".skillset", "changes", "history.jsonl"), "utf8")
    ).resolves.toBe("history\n");
    await expect(
      readFile(join(root, ".skillset", "changes", "releases.jsonl"), "utf8")
    ).resolves.toBe("release\n");
    await expect(
      readFile(join(root, ".skillset", "changes", "state.json"), "utf8")
    ).resolves.toBe("{}\n");
    await expect(
      readFile(
        join(root, ".skillset", "changes", "pending", "abc123def456.md"),
  ...
  it("refuses pending entry collisions before writing", async () => {
    ...
    await writeFile(
      join(root, ".skillset", "changes", "abc123def456.md"),
      "new pending\n",
      "utf8"
    );
    ...
    expect(result.stderr).toContain(
      "refusing to overwrite existing pending change entry .skillset/changes/abc123def456.md"
    );
    await expect(
      readFile(
        join(root, ".skillset", "changes", "pending", "abc123def456.md"),
    ...
    ).resolves.toBe("old pending\n");
    await expect(
      readFile(join(root, ".skillset", "changes", "abc123def456.md"), "utf8")
    ).resolves.toBe("new pending\n");
  });
  ...
  it("refuses ambiguous config and manifest merges", async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain(
      "cannot merge duplicate top-level key compile"
    );
    await expect(
      readFile(join(root, ".skillset", "config.yaml"), "utf8")
    ...
    await expect(
      readFile(join(root, ".skillset", "skillset.yaml"), "utf8")
    ).rejects.toThrow();
  });
  ...
  it("supports dry-run output without mutating files", async () => {
    ...
    await writeFile(
      join(root, "changes", "pending", "abc123def456.md"),
      "pending\n",
      "utf8"
    );
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stderr).toContain(
      "move changes/pending/abc123def456.md -> changes/abc123def456.md"
    );
    ...
    await expect(
      readFile(join(root, "changes", "pending", "abc123def456.md"), "utf8")
    ).resolves.toBe("pending\n");
    await expect(
      readFile(join(root, "changes", "abc123def456.md"), "utf8")
    ).rejects.toThrow();
    await expect(
      readFile(join(root, ".skillset", "changes", "abc123def456.md"), "utf8")
    ).rejects.toThrow();
  ...
  it("moves dedicated root change state into the source root", async () => {
    ...
    await expect(
      readFile(join(root, "changes", "history.jsonl"), "utf8")
    ).rejects.toThrow();
  });
