...
describe("changeset guard", () => {
  test("requires an active changeset for published package payload changes", () => {
    ...
    expect(result.packageFiles.map((file) => file.path)).toEqual([
      "apps/skillset/src/cli.ts",
    ]);
    expect(result.diagnostics[0]).toContain(
      "Package-facing changes require a .changeset/*.md entry"
    );
  });
  ...
  test("treats the scoped CLI manifest as package-facing", () => {
    ...
    expect(
      isPackageAffectingPath("apps/native-darwin-arm64/package.json")
    ).toBe(true);
  ...
  test("reports changesets that mix published and ignored private packages", async () => {
    const root = await createTestFixtureRoot("skillset-changeset-guard-");
  ...
  test("command rejects mixed release entries with an actionable diagnostic", async () => {
    const root = await createTestFixtureRoot("skillset-changeset-command-");
  ...
  test("classifies private-only, public-only, explicit ignore, and versioned private entries", async () => {
    ...
    await expect(
      findMixedChangesetReleaseEntries(ignoredRoot)
    ).resolves.toEqual([
      {
    ...
    await expect(
      findMixedChangesetReleaseEntries(versionedRoot)
    ).resolves.toEqual([]);
  });
  ...
  test("blocks active changesets when only repo machinery changed", () => {
    ...
    expect(result.diagnostics[0]).toContain(
      "Changeset entries are only for published package payload changes"
    );
  });

  test("allows correcting existing active changeset metadata", () => {
    const result = evaluateChangesetGuard([
      { path: ".changeset/release-plan.md", status: "M" },
    ]);

    expect(result.ok).toBe(true);
    expect(result.packageFiles).toEqual([]);
    expect(result.changesetFiles.map((file) => file.path)).toEqual([
      ".changeset/release-plan.md",
    ]);
  });
  ...
  test("does not treat tests as package payload changes", () => {
    expect(
      isPackageAffectingPath("apps/skillset/src/__tests__/skillset.test.ts")
    ).toBe(false);
    expect(
      isPackageAffectingPath("packages/core/src/__tests__/build-result.test.ts")
    ).toBe(false);
    expect(
      isPackageAffectingPath("packages/lint/src/rules/name-directory.ts")
    ).toBe(true);
  });

  test("requires release intent for provider and schema contract changes", () => {
    expect(
      isPackageAffectingPath("packages/registry/src/schema-snapshots.ts")
    ).toBe(true);
    expect(isPackageAffectingPath("packages/registry/src/migrations.ts")).toBe(
      true
    );
    expect(
      isPackageAffectingPath(
        "packages/registry/src/__tests__/snapshots.test.ts"
      )
    ).toBe(false);
    expect(isPackageAffectingPath("packages/schema/src/contracts.ts")).toBe(
      true
    );
    expect(isPackageAffectingPath("packages/schema/src/validate.ts")).toBe(
      true
    );
    expect(
      isPackageAffectingPath("packages/schema/src/__tests__/schema.test.ts")
    ).toBe(false);
    expect(isPackageAffectingPath("packages/toolkit/src/runtime.ts")).toBe(
      true
    );
    expect(
      isPackageAffectingPath("packages/toolkit/src/__tests__/runtime.test.ts")
    ).toBe(false);
    expect(
      isPackageAffectingPath(
    ...
    expect(
      isPackageAffectingPath("docs/reference/examples/skill-frontmatter.yaml")
    ).toBe(false);
  });

  test("ignores deleted changesets so cleanup branches can remove mistakes", () => {
    expect(
      isActiveChangesetEntry({ path: ".changeset/old.md", status: "D" })
    ).toBe(false);
    expect(
      isActiveChangesetEntry({ path: ".changeset/new.md", status: "A" })
    ).toBe(true);
  ...
  test("parses both name-status and path-only changed file lines", () => {
    ...
    expect(
      parseChangedFileLine("modified\tpackages/core/src/build.ts")
    ).toEqual({
    ...
    expect(
      parseChangedFileLine(
        "R100\tpackages/core/src/old.ts\tpackages/core/src/new.ts"
      )
    ).toEqual({
    ...
    expect(
      parseChangedFileLine("docs/development/package-releases.md")
    ).toEqual({
...
async function writeReleaseBoundaryFixture(options: {
  ...
  const root = await createTestFixtureRoot("skillset-changeset-boundary-");
  await mkdir(join(root, "apps/skillset"), { recursive: true });
