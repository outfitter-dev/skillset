...
describe("SET-489 structural path extraction", () => {
  ...
  test("quoted variable prefixes join trailing path segments", () => {
    for (const prefix of [
      '"$REPO_ROOT"',
      '"${REPO_ROOT}"',
      "'$REPO_ROOT'",
      ...
      expect(rules(`cat ${prefix}/packages/core`)).toEqual(["internal-package"]);
      expect(rules(`cd ${prefix}/docs/development`)).toEqual(["development-docs"]);
      expect(rules(`cat ${prefix}/scripts/private.ts`)).toEqual(["internal-script"]);
    }
    for (const path of [
      '"$HOME"/packages/core',
      '"${HOME}"/scripts/private.ts',
      'myrepo/"$ROOT"/packages/core',
      '"$ROOT"/myrepo/packages/core',
      '"myrepo/$ROOT"/packages/core',
